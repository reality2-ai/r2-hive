...
# Stage BOTH manifests and BOTH lockfiles — the wasm crate is workspace-
# excluded so it has its own Cargo.lock, regenerated by the wasm build above
# (hive-codex finding: omitting it committed manifest/lock skew and left the
# tree dirty while printing DONE).
git add Cargo.toml Cargo.lock crates/r2-hive-wasm/Cargo.toml crates/r2-hive-wasm/Cargo.lock
...
# Honesty guard: DONE must mean the pin surface is fully committed — any of
# the four pin files still dirty here means the bump is NOT clean.
if ! git diff --quiet -- Cargo.toml Cargo.lock crates/r2-hive-wasm/Cargo.toml crates/r2-hive-wasm/Cargo.lock; then
  echo "ERROR: pin files still dirty after commit — bump is NOT clean; investigate before pushing."
  exit 1
fi
