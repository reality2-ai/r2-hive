# RESUME-archive — r2-hive (hive-worker)
...
# ARCHIVED FROM RESUME.md 2026-07-19 — sections dated 2026-07-10 and earlier
Moved verbatim (no edit) to keep RESUME.md under the per-turn context cost. Nothing here is live state; live state stays in RESUME.md.
...
# ARCHIVED FROM RESUME.md 2026-07-19 (pass 2) — blocks already self-labelled ARCHIVE/superseded
...
=========================================================================
# ARCHIVED SNAPSHOT (v4→v8.7 full cycle history) — moved from RESUME.md 2026-07-25
# (RESUME.md exceeded 64KB fleet-doctor limit; compacted to current-only. Full detail below.)
=========================================================================
...
# ARCHIVED 2026-07-27 (night) — X1 baked-persona OTA campaign: full derivation trail

> Moved out of RESUME.md to restore it to one concise current snapshot (RESUME.md carried five
> stacked `(superseded)`/`(detail)` blocks and a section header still reading "awaiting grant"
> after act 1 had already landed). **Every fact below is preserved verbatim as written at the time.**
> The live/current state, digests and standing constraints remain in RESUME.md; this is the
> blow-by-blow derivation — attestation reasoning, the A-vs-B delta accounting, the tree-relationship
> proof, the retracted legacy-leg debate and its corrections.
...
- **★ FLASHABLE .bin — RULED (supervisor 2026-07-27): option (a), APP IMAGE not merged; THREE parties — Roy PRODUCES (runs the one save-image via `!`, ungated), hive ATTESTS, composer WRITES.** Granter (supervisor) must NOT produce what it pins; hive is gated; ⇒ Roy produces = cleaner separation than the original plan (arrived at because the gate over-matched). App-only (ota_0 @0x20000), no merge (merge would write bootloader+table from 0x0 = forbidden). Artifact: `~/x1-otav3-A-baked-app.bin`. **★ PRODUCED by Roy + ATTESTED by hive (independent, 2026-07-27):** size **857088 B**, **sha256 `0431d07c16cc9fa89708df5bff270ba772819ae9abce8308e81bb3d2a55a6963`** (computed without supervisor's number — two-way independent check). Blob byte-search ON THE .bin: 336 B found at offset **0xa904** (single occurrence), extracted-336B sha == blob `243ab04…426e` EXACT; tg_pk `4e2a9a30…7706` ×2 at 0xa9ba/0xaa2e both inside `[0xa904,0xaa54)`; tail non-zero (not padded). **DROM prediction HELD, measured:** esp image magic 0xE9, 5 segs; persona in **seg0 = DROM/.rodata** (load 0x3c000020) at vaddr **0x3c00a904 = IDENTICAL to the ELF vaddr** — the image only reframes it. NOT merged (seg0 is DROM, not a bootloader), NOT padded (857088/1966080). ELF 0xb904 → .bin 0xa904 (−0x1000) = esp-image framing only (ELF page-aligns file offset, image packs seg0 at 0x20). **AWAITING supervisor's digest compare → fill grant → composer writes 857088 B at literal 0x20000 app-only** after confirming ota_0 from X1's own bootloader. No-hand-roll refusal accepted on record. Prior HELD detail retained below.
  - **(c) rationale (supervisor):** granter (supervisor) must NOT produce the artifact it pins; hive is gated; composer is the writer — so Roy-produces gives THREE parties for THREE roles (produce/attest/write). Supervisor verified the on-disk ELF sha256 == `4e595a45…49ac8` independently before handing Roy the command.
  - **★ PARTITION-TABLE DISCREPANCY (supervisor finding 2026-07-27) — NOT a brick risk, both lanes use dfr1195's:** my `dfr1195/partitions.csv` (sha256 `e0e49127…627c`: ota_0 @0x20000 size 1.875 MiB, ota_1 @2 MiB, r2cfg @0x11000) ≠ composer's staged `xiao-wio-sx1262` template (ota_0 @0x20000 size **3 MiB**, ota_1 @3.125 MiB, phy_init @0x11000). Ruled safe: the receiver resolves the target slot via ESP-IDF partition API (inactive slot, AppPartitionSubType) = follows the FLASHED table, not a compiled constant; ota_0 @ 0x20000 is the one value both agree on + the only one act 1 needs. **Consistency: both lanes (Roy's produce + composer's write) use the dfr1195 table** (FYI sent composer, sha-pinned). **My attestation is unaffected:** a non-merged app image is table-INDEPENDENT (the table only size-checks); app loadable ≈ 1048371 B (0xfff33) fits BOTH slots ⇒ same bytes either way.
- **★ (superseded by the ruling above) FLASHABLE .bin — HELD, STOPPED ON THE INSTRUMENT (2026-07-27, even AFTER Roy authorised):** hive CANNOT produce the .bin and the gate deny is CORRECT-BY-DESIGN, not a bug. `espflash save-image` (pure file-write) is hard-denied by the PreToolUse gate `claude-fleet/hooks/auto-approve.sh` (`hs_bash` keyword-matches `espflash`); no ungated fallback (esptool absent). The ONLY sanctioned pass = `_hs_authorized()`: a live `.fleet/flash-authorization` whose `artifact`+`target` the command must name. **The current grant (rewritten by supervisor 19:00) is board-write-shaped: `artifact=PENDING-HIVE-DIGEST-DO-NOT-MATCH`, `sha256=0…0`, `target=X1`, `OPERATOR: composer` — DELIBERATELY unmatchable, "fails closed by content … the supervisor fills those two fields when hive attests … THE OPERATOR NEVER FILLS THEM … MUST NOT RENAME AN ARTIFACT TO FIT THIS FIELD."** ⇒ **CIRCULARITY:** supervisor's message asks hive to PRODUCE the .bin, but the field that would let the gate pass is only filled AFTER hive attests the .bin — hive-as-producer is closed by the instrument. **Held on the file, refused the message** (the doctrine the grant file itself ratifies). Refusals, named: no field-fill (forge), no rename-to-fit, no `FLEET_FIRMWARE_GATE=off`, no wrapper/ssh-launder, no hand-rolled image (false pin) — [[espflash-trips-firmware-gate]], [[espflash-gate-bypassed-by-file-and-remote-exec]], [[attest-baked-bytes-and-sha-pin-handoff]]. **Escalated a/b/c:** (a) composer produces the .bin (named operator, has an espflash path) → hive attests; (b) supervisor writes a FILE-shaped save-image authorization (artifact=ELF basename, target=output path, not X1) → hive produces+attests; (c) Roy runs the one save-image. Verbatim cmd ready: `espflash save-image --chip esp32s3 --partition-table platforms/dfr1195/partitions.csv ~/x1-otav3-A-baked.elf ~/x1-otav3-A-baked-app.bin` (app-partition-only, explicit PT, no merge = grant act-1 shape; app offset 0x20000). On any produced .bin: report sha256+size, RE-RUN blob byte-search ON THE .bin (checked not inherited), STOP+report if padded/absent/offset-wrong.
...
- **★★ ACT 2 BLOCKED — ANTI-ROLLBACK RECORD HAS NO MAGIC (composer measured on X1 metal 2026-07-27; hive VERIFIED at pinned r2-core `4b4a71e5` by `git show`, core-owned):** `read_anti_rollback` (main.rs:8076-8088) has **NO validity check** — it reads 8 B at `ANTI_ROLLBACK_OFFSET 0x18000` and only maps `0xFFFFFFFF`→0, so any non-erased bytes become `(current_seq, authority_epoch_floor)`. X1's 0x18000 holds **ESP-IDF app text** (composer's granted 4096-B read: ASCII `equired eFuse bits not burnt`, 3951/4096 printable, ZERO 0xFF ⇒ the record was never written; leftovers from an earlier full flash whose app began below 0x20000 — an app-only write at 0x20000 never clears them). Firmware therefore computes **current_seq = 1769304421** (`equi` LE) and **floor = 543450482** (`red ` LE) — arithmetic re-verified by hive; composer's decimals exact (their hex notation was the big-endian rendering). ⇒ **every OTA silently refused with reason 6 StaleSeq** (`reject_reason` lib.rs:814; gate lib.rs:629). **The in-file control:** `read_ota_pending` (:8104-8113) DOES validate `OPND` magic and returns `None` — same file, two sectors apart; the crash-safety record self-validates, the security-critical one does not.
  - **hive ADDITION 1 (changes the fix design): BOTH halves are poisoned.** main.rs:8364/8479/8780 feed `current_seq: cur_seq` AND `authority_epoch_floor: floor`; lib.rs:498 rejects `ep < authority_epoch_floor` as **RevokedAuthority (12)**. A seq-only repair just trades reason 6 for reason 12 ⇒ the fix MUST invalidate the WHOLE record (composer's magic+length shape does this correctly).
  - **hive ADDITION 2: the code's own comment states the assumption that fails.** The A7/A8 triage note above `ANTI_ROLLBACK_OFFSET` justifies the 0x15000→0x18000 move with *"A board carrying an old 0x15000 floor restarts at 0 here — acceptable pre-deployment"* — true ONLY if the new sector reads ERASED. The same comment notes the region "survives an app-reflash": so does the garbage. The migration argument IS the defect, in writing.
  - **hive ADDITION 3: do NOT unwedge by climbing over the garbage.** `write_anti_rollback` commits `cs.max(seq), cf.max(floor)` at confirmed-boot (:4170-4171) ⇒ a package signed with seq > 1.77e9 would make the garbage-derived floor **permanent** on that board. Firmware validation is the fix; a seq-climb is a trap.
  - **★★ ACT 1 ALREADY RAN AND PASSED (supervisor state-correction 2026-07-27 — hive's record was BEHIND and said "awaiting"; corrected here).** composer wrote **857088 B at literal 0x20000, app-only, exit 0**. **Positive control passed on THREE limbs:** MAC-fallback hive_id `00fbe520` GONE; identity `cf1bf564` exact in all four emissions; build_id flipped to `otav3.A.baked.0727`. Then the single-variable proof: **reason 4 → reason 6** on a byte-identical payload, same signer, same bearer. ⇒ **`baked_persona` FIXED the signer gate on metal; reason 4 is gone.**
  - **★ WHAT THAT UPGRADES IN HIVE'S OWN CLAIMS (stated precisely — this is the static-vs-runtime axis):** hive's attestation was **STATIC** (blob verbatim in `.rodata`, PT_LOAD-loadable, tg_pk in region, byte-search on the `.bin`). The three-limb metal read is what **UPGRADES it to a RUNTIME claim** — the bake took **ON THE BOARD**, not merely in the ELF. Two different claims; only now are both discharged ([[identity-verified-is-not-function-verified]], [[dont-let-a-fix-land-on-an-unconfirmed-mechanism]]).
  - **OPERATIONAL IMPACT:** **ACT 2 CANNOT COMPLETE on X1** — reason 6 fires BEFORE the signer gate, so the reason-4 fix is now MASKED by reason 6. **Supervisor ruled (iii): NO 0x18000 clear tonight** (CCR1 one sector away ⇒ a length error is destructive; hive's reason-12 finding means a naive clear may not even be the right repair; and an unvalidated reader is a defect on EVERY board, so clearing X1 hides it on one). core rules on the firmware fix; Roy rules in the morning. **The de-provision falsifier is DEFERRED, NOT LOST, retired UNSPENT, and is NOT scored either way from tonight** — hive's reason (it needs the unbaked payload to boot) plus supervisor's independent and stronger reason (it would strand the board identity-less).
  - **★ CLASS AUDIT — `read_anti_rollback` is an OUTLIER, not a systemic gap (hive, swept every config-plane raw-offset reader @4b4a71e5):** **9 of 11 already VALIDATE a BE-u32 tag in bytes[0..4] and `return None` on mismatch** — `read_human_label` 0x1B000 (LBL1), `read_provisioned_tg` 0x14000, `load_link_key` 0x1C000, `read_role_profile` 0x17000, `read_dest_nvs` 0x16000, `read_mask_nvs` 0x15000, `read_coarse_checkpoint` 0x1D000, `read_ota_pending` 0x1A000 (OPND), `read_rollback_record` 0x1E000. **Only TWO do not:** `read_anti_rollback` 0x18000 (the fired, security-critical one) and `read_board_profile` 0x13000 (no tag; cosmetic — garbage yields wrong screen/LED-polarity, not a bypass). ⇒ **the fix is conformance to an idiom the file already keeps**, not a new mechanism. **METHOD DISCLOSURE:** hive's first pass scored 10/11 — the audit regex counted `read_board_profile`'s `b[0] != 0x00` as a validity test when it is data interpretation; caught by eye-checking 3 MAGIC-positives against source before sending. 9/11 is the corrected, spot-verified count ([[marker-grep-cannot-see-comments]] applied to hive's own instrument).
  - **★ FIX SHAPE hive supplied to core (edit is core's; hive edits no r2-core):** a naive magic is **NOT safe alone** — it invalidates every legitimately-written legacy record, so boards that DO hold a real floor drop to floor=0 and **the fix itself opens a downgrade window**. **Discriminator (rests on composer's zero-0xFF negative control):** `write_anti_rollback` writes 8 B into a freshly ERASED 4 KB sector ⇒ a real record has `0xFF` from byte 8 on; foreign bytes do not. So: tag present ⇒ new format; else all-0xFF tail ⇒ legacy, trust it; else ⇒ **(0,0)** (X1's case), all-or-nothing per the reason-12 finding. Writer emits the tag (OPND idiom: magic BE ‖ payload LE). **Assert the erase as a PRECONDITION, not a comment** — leg (b) is load-bearing on it, and an unstated assumption is exactly what caused this defect. Honest limit: the 0xFF-tail test is probabilistic (widen N), a migration bridge — the tag is the permanent mechanism.
  - **★★ SHARPER FORM (supervisor, hive-VERIFIED verbatim @4b4a71e5): THE THREAT IS WRITTEN DOWN IN THE SAME FILE, TWICE, on the magics themselves** — `:6203 COARSE_CHECKPOINT_MAGIC = 0x4354_3100; // "CT1\0" — guards an erased/foreign sector reading as 0` and `:8147 ROLLBACK_REC_MAGIC = 0x5242_4B31; // "RBK1" — guards an erased/foreign sector reading as valid`. The author NAMED this exact failure mode and implemented the guard for the neighbouring record while leaving the floor bare. **Worth more than the 9/11 count.** hive's sharpening: **RBK1 guards the ROLLBACK DIAGNOSTIC (0x1E000) while the SECURITY FLOOR two sectors away is unguarded — the guarded neighbour is the less critical one.**
  - **★★ hive ARGUES AGAINST ITS OWN LEGACY LEG (writer trace, prompted by supervisor's permissive-default rule):** `ANTI_ROLLBACK_OFFSET` occurs exactly 3× — const `:8074`, read `:8080`, write `:8097` = **ONE reader, ONE writer**; `write_anti_rollback` has **ONE call site (`:4171`)**, reachable only via `New|PendingVerify` → `ota_health_check()` → `set_current_ota_state(Valid)` → `read_ota_pending()==Some`. ⇒ **a legitimate legacy record can exist ONLY on a board that COMPLETED A CONFIRMED OTA.** **RECOMMENDATION: DROP the legacy leg** — tag ⇒ new format, ANYTHING else ⇒ `(0,0)`. *The best answer to "a permissive default must be declared and argued" is to not need one*; it also removes the falsifier supervisor required, since no permissive branch remains. **Reduces to ONE question hive will NOT answer from its own lane (no global negatives): has ANY board ever completed a confirmed OTA?** (round trip never completed this campaign — 4, now 6; X1's zero-0xFF proves its writer never ran). Supervisor's/Roy's call. **specs' argument cuts hive's leg too, conceded:** `equired` is a text-buffer tail and `0x69757165` a plausible u32, so no range/sanity check could catch it — only **record IDENTITY** authenticates; **hive's 0xFF-tail test is a SHAPE HEURISTIC, NOT AUTHENTICATION**.
  - **IF the leg survives — the three answers supervisor required (sent core):** **(1) direction argued, not inferred:** `(0,0)`-for-everything fails toward a **remotely exploitable downgrade window** (exactly the threat the record exists to stop); trust-legacy fails only under pre-existing **flash-write** access (anti-rollback already lost) or coincidence, and a bogus-HIGH floor costs **availability** (the X1 mode), not integrity. **(2) five KAT vectors**, each hitting a named branch — tagged→NEW; 8 non-0xFF + 0xFF-from-byte-8→LEGACY; **8 non-0xFF + ONE non-0xFF byte in the tail→(0,0) [negative control, proves the tail is READ not assumed]**; full app-text sector (X1 shape)→(0,0); all-0xFF→(0,0) via `z()`. **(3) the foreign-sector-ending-in-0xFF case answered STRUCTURALLY** (low probability is not a design answer): wrong-fire requires the predecessor app's data to end **EXACTLY at sector byte 8** — end later ⇒ app bytes in the tail ⇒ (0,0); end earlier ⇒ bytes 0..8 are themselves 0xFF ⇒ (0,0) via `z()`. Segments pad/align to ≥16 B, so ends land on 0x18000 or 0x18010 and byte 8 is **unreachable ⇒ excluded by ALIGNMENT, not luck** — **and that alignment must be ASSERTED, not assumed**, else state the residual as accepted-and-why or drop the leg (handing over a fix resting on an unstated precondition would be the very defect being repaired).
  - **Companion (supervisor): the night's recurring shape = A WRITTEN ASSUMPTION STANDING IN FOR A CHECK** — the 0x15000→0x18000 comment assuming an erased destination; main.rs:4077 citing another platform's `sdkconfig` for a safety backstop; and hive's own audit regex asserting a property it never tested. **The comment makes it worse: the next reader takes it as verified.**
  - **★★ RESOLVED — LEG RETIRED, FIX IS TAG-ONLY (composer answered with evidence 2026-07-27; doc corrected at `33f1c66` BEFORE core read both).** **No board is known ever to have committed a floor**, on three grounds: **(a)** the source's own migration sentence, **(b)** `write_anti_rollback` reachable only after a completed confirmed OTA (hive's writer trace), **(c)** zero evidence any occurred — archive null **with a FIRED positive control**, X1's zero-0xFF, campaign never completed. composer's archive "counter-example" (a `seq=1 floor=3` confirm line) **DISSOLVED: it was the search's own synthetic positive control matching itself**; both real result files are ZERO BYTES ⇒ the corpus holds no board-emitted confirm line at all. **The null is admissible precisely BECAUSE the control fired** ([[never-conclude-from-a-null]]). *(That search was supervisor's, not hive's — hive claims no credit for the counter-example.)*
  - **★★ GROUND CORRECTED (supervisor 2026-07-27, hive ACCEPTED — hive had the grounds in the wrong order): DROP THE LEG ON THE AUTHENTICATION ARGUMENT, NOT ON THE GLOBAL NEGATIVE.** hive let the drop-recommendation lean on *"has any board ever confirmed an OTA"* — a negative hive explicitly declined to assert and then built on anyway. specs' argument kills the leg **independently**: a 0xFF-tail test is a **SHAPE HEURISTIC, not authentication**, failing the SAME criticism that condemns the reader ⇒ accepting an untagged record because its tail looks erased **re-commits the original defect in weaker form**. **TAG-ONLY IS CORRECT EVEN IF LEGACY RECORDS EXIST.** Core brief amended to that ground.
  - **TWO CLAIMS NOW STATED SEPARATELY (only one is settled):** **(1) SETTLED** — design is tag-only, on authentication grounds. **(2) OPEN** — whether tag-only opens a real downgrade window on deployed boards. Honest wording (supervisor's, not to be upgraded): **"NO LEGACY RECORD HAS BEEN OBSERVED ANYWHERE, AND NO EXHAUSTIVE SEARCH HAS SUCCEEDED"** — do NOT read as "none exist." **A direct D4 0x18000 read is QUEUED FOR MORNING** (needs a grant; Roy asleep; hive is NOT requesting one).
  - **★★ ARCHIVE LIMB WITHDRAWN ENTIRELY (composer @ doc `ed5612f`, after hive's control-placement question) — it contributes NOTHING in either direction.** (1) the positive control sat under `/tmp` while the search root was the home dir ⇒ it proved **PATTERN + TOOL only, nothing about corpus reachability** (hive's distinction, confirmed); (2) **TWO of three runs NEVER EXECUTED** (`timeout` cannot exec a shell builtin ⇒ rc=127) — the 0-byte outputs were **no-results, NOT nulls**, and their emptiness had been reported as data **without checking the writer's exit status**; (3) the one run that did execute **timed out (rc=124) after 167 files, ALL of them our own session artifacts** — the walk started in a dotfile dir and was still inside it when killed, so it **provably never reached where board captures are kept**. ⇒ even the narrowed *"no matching line in the searched paths"* was too strong.
  - **★★ "AN ARCHIVE THAT WAS NEVER AN ARCHIVE" — the question was UNANSWERABLE IN PRINCIPLE (supervisor re-ran the search properly, reachability control planted INSIDE the corpus and excluded from the count):** no confirm line — **but the corpus is TWO FILES, both from today, both ONE board.** That is the entire preserved board-capture archive on this machine; **every earlier capture died with its session scratchpad.** ⇒ **residual-risk item (2) can NEVER be closed from logs, only from SECTORS.** ★ **And the three instrument bugs MASKED that:** had they not fired, the run would have produced a clean, fast, **CONFIDENT null from a near-empty corpus** — and been believed.
  - **★★ STANDING POSTURE — BINDS NOW (supervisor-DECIDED 2026-07-27; authority = supervisor; status = IN FORCE, not advice): NO BRIEF, LEDGER ENTRY OR REPORT MAY CITE A LOG ARCHIVE AS IF ONE EXISTS.** Until a durable archive exists, retrospective device questions are answered by **MEASURING HARDWARE** or marked **UNANSWERABLE**. **"The logs show no X" is not a weak claim — it is NOT A CLAIM.** (hive's *either-fix-it-or-stop-citing-it* adopted verbatim.) **Policy DECIDED on the durable capture directory: YES — per-board, persisted outside session scratchpads, APPENDED to, never recreated. Owner composer (bench + capture tooling). NOT tonight** (new work, nothing pressing, composer under a nothing-touches-a-board order) — **queued for morning with Roy's other three.**
  - **✔ hive SWEPT ITS OWN RECORDS against the new posture (measured, not asserted):** patterns *the logs show/confirm/prove · log archive|history · archive shows|confirms · no … in the logs · grepped the logs*; denominator **214 tracked files**, `docs/archive/` excluded as already-deprecated. **TWO hits, ZERO violations** — `RESUME.md:142` is hive's own gap-raising text (describes the problem, cites nothing); `RESUME-archive.md:1330` ("composer's first-SDU log shows the OST split") was a **CONTEMPORANEOUS console read used live as a discriminator, not a retrospective archive grep** — a measurement when made. **⚠ BUT it carries the currency cost the policy is about: that capture DIED WITH ITS SESSION, so the claim is now PERMANENTLY UNVERIFIABLE.** Not rewritten (the archive records what was believed and on what basis — that is its job); flagged so it is never treated as re-checkable ([[identity-verified-is-not-function-verified]] on currency).
  - **★★ supervisor's SHARPENING of hive's inversion, banked as the rule (better than hive's original): AN INSTRUMENT FAILURE IS LOUD; AN EMPTY CORPUS IS SILENT ⇒ THE DANGEROUS NULL IS THE ONE THAT COMES BACK QUICKLY AND CLEANLY**, because nothing about a tidy `0 results` invites *"what did you actually search?"*. **The bugs were the only reason anyone looked at the denominator.** Practice: treat a clean null as the case NEEDING the denominator check, and report corpus size alongside every null. **Forcing asymmetry (hive's, adopted):** a live board can be measured; **a REFLASHED board cannot be asked about its past at all** — and boards are reflashed weekly, so **every reflash is a permanent loss of answerable history** until the capture directory lands.
  - **(superseded by the ruling above) ⚠ STANDING GAP RAISED BY hive (flagged, NOT requested; morning queue; owner = composer for tooling, policy = supervisor):** captures not surviving their session makes **ANY** retrospective device question unanswerable — *did this board ever do X, when did Y start, what did Z print before the reflash* — and a **reflashed board cannot be asked about its past at all**. Proposal: a durable per-board capture directory, appended to rather than recreated (cheap, no protocol impact; converts "unanswerable in principle" into "grep it"). **If judged not worth it, the honest posture is that retrospective device questions get answered by MEASURING HARDWARE or not at all — and briefs must stop implying a log history exists to be searched. Either fix the archive or stop citing it.**
  - **⚠ hive FLAGGED composer's "and they are enough" — ENOUGH FOR WHICH CLAIM (the agreeable direction, named for that reason):** **(1) THE DESIGN (tag-only) — settled, and never rested on this evidence at all** (supervisor moved it onto specs' AUTHENTICATION ground precisely so no negative carries weight). **(2) THE RESIDUAL RISK — NOT settled by writer trace + X1:** the **writer trace narrows the POPULATION** ("only a board that completed a confirmed OTA can hold one") but **does not show that set is EMPTY — it is a structural fact about the code, not a census**; and **X1 is ONE board, directly measured — D4, D5, RAK UNMEASURED.** Supervisor's wording stands unchanged: **"no legacy record has been observed anywhere, and no exhaustive search has succeeded."** D4's queued read is the thing that would move it.
  - **★ supervisor REPORTED THEIR OWN INSTRUMENT FAILURE rather than the absence — three in a row, each an empty result that reads as a clean negative:** (1) `grep | head` — read `0` as grep's rc when it is **head's**; (2) `timeout <shell-builtin>` — timeout cannot exec a builtin ⇒ **rc=127, the search NEVER RAN** and returned nothing; (3) run correctly ⇒ **rc=124, TIMED OUT at 500 s, incomplete**. **And the corpus is SELF-POLLUTING:** of 167 pre-timeout hits, essentially all are our own agent transcripts/paste-caches/file-history — *us discussing the string, not boards printing it*. ⇒ **THE PROXY WAS WRONG, NOT MERELY BROKEN** — "does the confirm string appear in logs" was never the question; fixing the instrument would have produced a better-executed wrong measurement. One bounded direct sector read settles what an unbounded grep over a polluted corpus cannot.
  - **✔ BOTH hive ADDITIONS ADOPTED INTO THE GRANT ITSELF (supervisor 2026-07-27), not merely the core brief — "it is an ORDERING hazard and grants are what gate action."** Grant text: *the fix MUST NOT auto-repair by writing a tagged floor=0 over untagged bytes; read-only handling until the population is characterised;* **NO grant will be written for a repair write before the D4 read is taken.** **STANDING ORDER: measure D4 → fix → any repair under its own grant.** Supervisor named the class: **THE REPAIR WOULD ERASE THE QUESTION** — not merely lose data; it destroys the only evidence of population size **on every board it lands on**, via **the most natural instinct anyone shipping a validator would have**, and *a fix that ships before the measurement makes the measurement unrepeatable and unfalsifiable*. **"Fail closed but not silent" adopted verbatim**, with supervisor's sharpening: **the countability property is worth more than the diagnostic itself** (the affected population becomes measurable in the field with no grant per board — recovering cheaply the very measurement a silent repair would have destroyed).
  - **(detail) hive ADDITION — URGENT CONSTRAINT ON THE FIX (sent core): THE FIX MUST NOT AUTO-REPAIR BY WRITING.** The natural *"write a tagged floor=0 so it cannot recur"* instinct would **DESTROY, on every board the fix lands on, the only evidence of the affected population's size** — including D4's sector if the fix ships before the queued read. **Read-only handling until the population is characterised**; self-repair is a separate, argued change later.
  - **★ hive ADDITION — FAIL CLOSED, BUT NOT SILENT.** `(0,0)` is the right VALUE and the wrong UX: a board silently dropping to floor=0 has lost downgrade protection with nothing to notice. The reader should **distinguish NON-ERASED UNTAGGED bytes from an ordinary erased sector and SAY SO** — boot diagnostic minimum, better a periodic **health field** — converting a silent security regression into an observable one **and making the affected population countable in the field without a grant per board**. (Caveat passed to core: pick the next genuinely free health key by re-reading the CURRENT map, not from anyone's memory including hive's — hive once proposed an "additive" key that collided in that exact map.)
  - **✔ CLOSED (composer landed both hive precisions @ `03beac9`, before core read):** ground (a) narrowed to the pre-move class verbatim (incl. hive's line *"a comment cannot exclude a class it does not mention"* and the *"acceptable PRE-DEPLOYMENT"* self-limit); **the WRITER TRACE is now the load-bearing ground**, archive demoted to corroboration; composer stopped calling them "three INDEPENDENT grounds" (unequal weight, only one covers the class that mattered); the over-read is recorded **as a correction note, not quietly fixed** — *"I over-read a text, inside the document about over-reading texts."* **Archive scope narrowed to the honest reading "no matching line in the searched paths"** — composer can see the control is in a session scratchpad and both result files are 0 B but CANNOT see the globs, so it is **pending supervisor** (whose search it was). ★ composer **declined to reconstruct it from supervisor's session transcript** — *mining another lane's working notes to settle a question you can ask is over-reach*; good cross-lane norm, endorsed. Falsifier stays as the named attack. **Nothing further owed by hive on this thread.**
  - **(detail) hive NARROWED composer's ground (a) — a comment cannot exclude a class it does not mention:** verbatim `:8073` *"A board carrying an old 0x15000 floor restarts at 0 here — acceptable pre-deployment"* is scoped to the **PRE-MOVE** class only. It says nothing about a board that confirmed an OTA **AFTER** the 0x15000→0x18000 move — which would hold a legitimate UNTAGGED record at 0x18000, **exactly the legacy class the leg existed for**. So the population claim rests on **(b)+(c)** for the class that actually mattered; conclusion unchanged, load-bearing ground changed. Asked composer to rewrite (a) as "pre-move class only" — otherwise the document re-commits, inside itself, the very shape it documents. (The sentence also self-limits: *"acceptable PRE-DEPLOYMENT"* — not a claim about deployed boards.)
  - **⚠ hive's open question on the archive null's SCOPE:** a positive control written to a **scratch file** proves the PATTERN fires and the TOOL works; it proves **corpus REACHABILITY** only if planted in the same path/glob the real artifacts occupy. If not, a zero result is equally consistent with *"the real logs were never in the searched path"* and the null carries a narrower scope than claimed ("no matching line in the searched paths"). Asked composer which. composer's stated limits (one machine; console output ephemeral so absence-of-log ≠ absence-of-event; one board measured, reading others unauthorised) accepted. **Falsifier named in composer's doc and endorsed by hive: a board whose 0x18000 holds a short record + all-0xFF tail RESURRECTS the leg.**
  - **General rule composer recorded from hive's concession (outlives this bug):** *an all-0xFF tail is a SHAPE; foreign data can exhibit it; **ONLY RECORD IDENTITY AUTHENTICATES**.* Applies to any looks-right test — **including one proposed in good faith**, which is why it is written down rather than quietly dropped. **Vindication framed accurately (composer's wording, hive will not soften it):** fail-closed-to-zero is upheld **by the structural argument, not because the first instinct was better reasoned**.
  - **(superseded) ⚠ CROSS-LANE INCONSISTENCY, FLAGGED (2026-07-27):** composer republished at `fe68312` with hive's migration discriminator recorded as **accepted design (a three-way table)** — but hive has SINCE RETRACTED it (writer trace ⇒ recommend tag-only). **composer's doc and hive's core brief currently disagree**; retraction sent to composer with the trace, so core does not read two conflicting recommendations. Also told composer their ORIGINAL "fail-closed-to-zero" instinct may be **vindicated, not overturned** — hive's objection (unsafe for real floors) holds only IF real floors exist. **The deciding question was ROUTED TO COMPOSER**, who holds the bench + flash history: *has any board ever completed a confirmed OTA (reached main.rs:4171)?* If no ⇒ leg dies, fix is tag-only; if a board can be NAMED ⇒ hive's leg stands and the three required answers become live. Evidence, not preference. (CT1/RBK1: composer re-derived independently = three lanes converging; already in core's brief, credit composer's, no action.)
  - **composer's durable write-up:** `orchestrator/bench/ANTI-ROLLBACK-FLOOR-DEFECT.md` @ r2-composer `be588aa` (branch `composer/ota-signed-stream`), public-safe (handle X1 only, no MACs/TG values). composer deliberately did NOT write or erase the sector — it would fix one board and hide the defect on every other.
  - **DISCLOSURE on hive's own artifacts:** images A (`4b4a71e5`) and B-lineage (`b25a21eb`) BOTH carry this defect (it is in the source they were built from). The attestations stand — the artifacts are exactly what they were attested to be — but neither can complete an OTA on X1 until the record is invalidated or the sector cleared. Blast radius: any device app-only-flashed over a differently-laid-out predecessor; **not X1-specific**. Owner = **core** (relayed by composer + hive's additions); hive edits nothing here.
- **★ A-vs-B .bin DELTA — FULLY ACCOUNTED, comment-only CONFIRMED, NOT a re-persona (supervisor finding 2026-07-27):** supervisor's `cmp -l` showed 121599 differing bytes (14%) + size delta +16 between A `0431d07c…6963` (857088 B) and B `965419ba…ac5d` (857104 B) — "fits neither a comment-only rebuild nor a 16-byte shift." **Resolved by measurement, all four asks + the confound:**
  - **(0) tooling confound EXCLUDED:** both bins carry a byte-identical extended header `ee000000090000000063000000000001` (chip_id=9, `hash_appended=1`), same magic/segcount/spi_mode/spi_speed/entry `0x4037877c`; both are 5-segment APP images (seg0=DROM, not a merged bootloader); both trailers self-consistent (embedded sha256 == sha256(image[:-32]), `(len-32)%16==0`). Same tool, same flags ⇒ the delta is SOURCE not tooling. **★ HARD PROVENANCE BINDING:** each bin's `esp_app_desc_t` embeds `app_elf_sha256` == the ELF hive attested, all 32 bytes (A `4e595a45…49ac8`, B `22e497cd…3bf3`) ⇒ Roy's bins are cryptographically bound to hive's attested ELFs.
  - **(1) +16 exact, by named bytes:** seg0 .rodata +16, seg1 DRAM −16, seg3 DRAM +8 (load 0x3fc9b9ac→0x3fc9b99c), seg2/.text and seg4/IRAM ±0 ⇒ segment data +8; trailer 36→44 B (+8) = pad 3→11, FORCED by the format (pad so `(len−32)%16==0` before the 1-byte checksum + 32-byte SHA). +8+8 = +16. No appended payload.
  - **(2) the 121599 = absolute-offset artifact + RELOCATION:** `cmp -l` compares at absolute offsets, so the +16 shifts everything after seg0 and scores identical bytes as differing (hence "last diff at 857088"). Segment-aligned = **63251**. Composition: **.text same length, 2511 differing words, ALL of them POINTERS (1891 .rodata + 620 DRAM), ZERO instruction words** — same code, relocated data; **.rodata 50.8% = relocation not modification**, proven 3 ways — 98.39% of A's 64-B .rodata blocks (1707/1735) exist verbatim in B; **string sets identical 1584/1584, 1582 shared, only diffs = compile time + build id**; 118 line-number u32s shifted +1..+12, **mode +8 = the comment commit's net +8 lines**. First diff runs = app descriptor (compile-time string @0x53/0x56, app_elf_sha256 @0x90). The missing hypothesis in the two-way test was **relocation**.
  - **(3) PERSONA MATCH (the stop-condition, passed):** A @0xa904, B @0xa950, each single occurrence, **both 336-B window sha256 == `243ab04…426e` == the blob, and identical to each other**. Offset delta +0x4c = the .rodata relocation. ⇒ **NOT a re-persona; #68 not engaged.**
  - **(4) tg_pk `4e2a9a30…7706`:** A count=2 (0xa9ba/0xaa2e), B count=2 (0xaa06/0xaa7a), all inside each bin's persona region; build-id exclusivity holds (A has only otav3.A, B only otav3.B) — no cross-contamination.
  - **A's pinned artifact NOT touched** (re-verified 857088 B / `0431d07c…6963`); no rebuild needed — structure answered the confound (and espflash is gated for hive anyway).
...
# ARCHIVED 2026-07-28 (compaction, pass 2) — g23 VALUE-SCRUB (LANDED, closed) + ROUTE-ORIGIN-1 GROUP_MGMT carve-out (core-owned)

**Relocated verbatim from RESUME.md, not deleted. Falsifiers and proof shapes travel WITH their items — an
archived item that keeps its title and loses its proof shape is exactly the loss the exemption existed to
prevent (supervisor, 2026-07-28). Split by AGE AND RELEVANCE, never mid-item.**
