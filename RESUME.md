...
**★ SEALED-RECOVERY LEG design input CAPTURED (RAK-MCUBOOT-OTA-PLAN.md §4.1/4.2, commit f7a2c75; supervisor-relayed Roy's RAK DFU doc 2026-07-11; NO action on the live spike):** stock RAK4631 = Adafruit UF2 (double-tap) + Nordic serial DFU (AT+BOOT→nrfutil usb-serial signed zip); **HW un-brick = `UART2_TX`→GND + RESET forces DFU APP-INDEPENDENT** (the computer-not-required re-commission-bypass primitive). **OPEN DESIGN Q for the slot-map stage: KEEP stock bootloader as recovery vs REPLACE with MCUboot serial-recovery (mcumgr/SMP); INVARIANT = preserve the UART2_TX→GND force-DFU jumper either way.** board-verify still applies (RAK doc says ttyACM0 but Alfred RAK=ttyACM1). 2nd ref (WisBlock bootloader README / adafruit-nrfutil SWD-restore) incoming — supervisor folds next. ⚠ NOTE: supervisor called this "task 60" but local #60 = hive-bin trail gap; the RAK recovery leg is under **#44** (MCUboot OTA) — filed there.
