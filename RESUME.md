...
## 🔥 XIAO EDGE-BRIDGE POWER-STANDBY (heat fix, Roy GO 2026-07-10, task #65) — PLAN reported, HELD on Roy scope + specs contract
Roy: the XIAO edge bridge runs hot. **PLAN (report-first, no deep impl yet): docs/XIAO-BRIDGE-STANDBY-PLAN.md (6a709e6).** Root cause GROUND-TRUTHED vs LIVE dfr1195-fw 8022c2e: SX1262 CONTINUOUS RX (SetRx RX_CONTINUOUS, r2-sx1262:800) = always-hot; STEP-4 duty-cycle NOT landed (advertised-not-enforced main.rs:388/1288/2816).
...
- **✅ SPEC CONTRACT RESOLVED to a low-churn shape (specs pre-loaded the anchors): NO NEW WIRE — reuse R2-WIRE §12.6 dc=Intermittent=2 + §3B.1 SCF (SCF_TTL_S=120, F2-proven, auto-buffers an Intermittent neighbour).** The real work = a normative R2-RUNTIME §3.2.1 role-model CARVE-OUT (reconciles bridge=AlwaysOn / "duty-cycled bridge invalid" ~line 222/308). **DRAFTED: docs/R2-RUNTIME-EDGE-BRIDGE-STANDBY-CANDIDATE.md (63df9c4), handed specs inline.** Carve-out: pure EDGE bridge (sole downstream = a presence-driven sink, no mesh-dependent downstream) MAY standby→dc=Intermittent. INVARIANT: standby legal IFF no dependent downstream expects it awake (matches §3B.1 will-dest-be-awake). DISTINCT from TRANSIT bridge (MUST stay AlwaysOn or strands the island; Intermittent-transit = violation). SIZING: standby wake_cadence < SCF_TTL_S (120s). Phone-presence transition = R2-RUNTIME §3.2.x runtime power-state.
- **DEPENDENCIES before path-1 impl:** (1) Roy scope-eyeball (reported supervisor); (2) specs RATIFY the §3.2.1 carve-out (draft handed, verify-then-ratify); (3) core add SetRxDutyCycle (r2-sx1262). **HELD until Roy approves scope + specs ratifies — do NOT start deep impl or trigger core driver work prematurely.**
