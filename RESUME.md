...
## 📡 RAK BLE bring-up (P4 = task #58/#44) — step-1 DONE (ground-truthed); frontier = on-metal steps 3–5 (GATED)
Ground-truthed the whole P4 state 2026-07-11 (rak4630-fw platforms/rak4630/BLE-PLAN.md §9/§10). **§9 step 1 (embassy git→0.7 crates.io migration, "THE 2a FOUNDATION TASK") is COMPLETE** — plan prose was stale ("currently uses embassy from git"); Cargo.lock resolves the whole 0.7-family from crates.io (0 git-embassy refs); default + `--features ble` (r2-ble descriptor @d80ebf7) both compile-green thumbv7em. Corrected the doc (dc6e3ed) so a takeover doesn't redo a big migration. **Frontier = steps 3–5 (partition apply / co-author BleHost binding WITH core on-metal / advertise) — ALL on-metal + core-co-authored → GATED on do-not-flash-RAK + core-live.** The nrf-sdc/nrf-mpsl/bt-hci binding deps arrive via r2-ble's `binding` feature, flipped on ONLY on-metal (pulls the Nordic blob). **⏳ ONE spec-first question ASKED core (blocks step-5 codec bytes + trouble-host advertise API): RAK repeater BLE beacon = LEGACY (31B, no bloom, esp32-parity) or EXTENDED (ext-adv, carries §7.2 bloom for neighbour-aware routing)?** The RAK weight-anchor (main.rs:814-830) currently encodes EXTENDED-with-bloom (all-zero placeholder identity, logged-only) but esp32 canon + FIELD-PLAN P4 both say LEGACY (`build_legacy_beacon`/`encode_advert`). **Anchor NOT edited pending the ruling** (editing either way bakes a profile). Reuse core's codec regardless; identity to fill = SAME as the LoRa beacon (`compute_rbid(derive_beacon_session_key(hk,hive_id),epoch)`, class_hash BE), NOT esp32's `rbid[0]=0xFF` L2CAP hack. **DO-NOT-ASSUME:** the codec bytes are core-provided (`r2-discovery::beacon`), not hive-authored — hive supplies identity + calls the codec.
...
esp32/IDF build = HIVE-owned; core writes it blind, hands me ONE diff (the r2-sx1262 split). Full picture: OTA-HARDENING-PLAN.md on the core branch. **AWAITING core's single patch; then I build + flash-verify on real esp32.** ⚠ CAVEAT: the esp32-IDF build currently builds on NEITHER box (owed via espup) — stand up the IDF toolchain before flash-verify (flagged core).
...
- **CRITICAL #2 — DUPLICATE early ota_health_check (main.rs:287 AND :906):** an early confirmer marks PENDING_VERIFY → VALID BEFORE the real §5 health check runs; then the canonical confirmer clears pending WITHOUT committing seq = a broken image is PERMANENTLY confirmed + rollback DISABLED. FIX (agreed): REMOVE the early confirmer — ONE confirm owner, after the real §5 checks, that commits seq.

## ⏳ HELD @specs: anti-rollback `staged_rollback_value` hook SHAPE not final (core update 2026-07-11)
core-codex REFUTED the `Option<u64>`-default-None hook sketch as silently-omittable (fail-OPEN if a sink forgets to override). Core raised a stronger NO-DEFAULT `fn staged_rollback_value(&self) -> Result<RollbackBinding, Self::Error>` (RollbackBinding = {ExplicitlyNotApplicable | Value(u64)}; parse/read failure MUST be Err) to specs and is HOLDING the land for their ruling. My hive-codex point that MCUboot SWAP/REVERT carries NO image-version backstop is FOLDED: under swap/revert my sink returns **ExplicitlyNotApplicable** (r2 signed seq = SOLE floor) — an EXPLICIT opt-out, not a silent None. **Do NOT bake the signature until specs rules; core sends the exact final signature then.** RAK ota.rs TODO already re-recorded to this shape (pending-specs).
...
- **✅ FIRMWARE STANDBY ARM AUTHORED + COMMITTED (dfr1195-fw 810573e), off-by-default `standby=["loraroute"]`.** RxenRadio overrides `listen_duty_cycle` (RXEN-high-once); lora_route_task calls `lora.set_rx_standby(3ms rx/5ms sleep)` sized for benchsf7 SF7 (symbol ~1.02ms, 8-sym preamble ~8.2ms, ~62% warm-sleep; SF12 needs re-size; sleepPeriod caps ~262ms). MCU light-sleep (1c) = precise documented TODO (esp-hal light_sleep+DIO1/USB-wake untestable off-metal; radio-duty-cycle alone = fastest heat win). **DEFAULT-image UNBROKEN: `cargo +esp check --features xiaobridge` (standby OFF) exit 0, no new warnings.** Standby arm compiles ONLY post-revendor.
