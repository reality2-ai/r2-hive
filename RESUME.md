...
- **M2 context (specs ruling, NOT my edit but the shared ABI axis — recorded):** specs is DROPPING min_core_abi from UpdateHeader — v3 UpdateHeader = abi_hash[8] + ensemble_semver[4] (135 B, not 137; reject-15 CoreAbiTooOld retired, 14 AbiMismatch stays). Reason: Ruling-2 exact-match means a v1 module can't depend on core semantics outside the ABI schema, so a band field has nothing to ride on until additive-minor compat (v2). ensemble_semver carries the band at composer's compat-matrix layer. core implements M2 against v0.47 (not v0.46). composer drops min_core_abi from recipe/v1 too.
- **✅ core CO-PINNED §12.4.3.1 — BOTH confirmed:** (a) native-image-136B acceptable + PREFERRED (r2-forge emits the byte-exact native image; its existing native AbiResult 136B KAT pins the wasm buffer too, zero drift); (b) the 8 export names/sigs match r2-forge codegen. **ONE refinement folded (r2-forge asserts on emit):** all INTER-FIELD PADDING bytes MUST be written ZERO (offsets [1..4] after tag + tail-to-136) — native repr(C,u8) leaves padding uninit, so pinning zero makes the 136B buffer fully-determined + KAT-pinnable + host reads deterministic (unused BRANCH region need not be zeroed — host reads only tag-selected fields). Plus: HOST MUST BE LITTLE-ENDIAN (native-LE image; wasm+x86/ARM all LE). Candidate updated r2-hive 4c1f4f8; sent specs both additions as the ratification addendum. core building the native vtable_for export codegen now (M1 follow-on, per specs ASK-2); the wasm-emit shares the same thunk/marshal.
- **NEXT (fully converged, awaiting specs write-access only):** specs lands R2-PLUGIN §12.4.3.1 from my inline text + the 2 additions (design ENDORSED + core CO-PINNED — no open design questions) → then BIND r2-wasm-host 1:1 (swap provisional_abi → the §12.4.3.1 abi module, rework spike to the real 8-export/136B-native-image convention) + interop-test a real r2-forge-emitted module against my host when core's wasm-emit is ready. Build-now meanwhile: B.4 TG-gated module-load gate (ABI-independent).
