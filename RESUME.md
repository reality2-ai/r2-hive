...
**✅ BLE KAT PINNED (core, r2-discovery canonical_rak_bench_ble_legacy_beacon_vector) — assert on-metal byte-exact:** `1bffffffb201007fce111165325a9abafe8ac1140000020000000000` (28B AD: Len1B FF CID-FFFF magicB2 ver01 flags00 rbid-7fce111165325a9a class-bafe8ac1 txpwr14=20 ac0000 buildclass02 +5×00). Identity == LoRa §8.1 beacon. encode_advert→24B inner=AD[4..28]; set_advert prepends 02 01 06. **⚠ AUTHORING CHECK: the vendored rak4630-fw/crates/r2-discovery LegacyBeacon I read earlier had NO build_class field, but core's KAT carries build_class@22 — so the vendored r2-discovery likely needs a RE-VENDOR to core's current beacon.rs (which has both the build_class field AND the pinned vector) before build_legacy_beacon emits this exact 28B. Verify at authoring; re-vendor r2-discovery if the struct lacks build_class.**
...
## ✅ RESOLVED→⏳ RE-VENDOR HELD: anti-rollback `staged_rollback_value` hook LANDED @core 7d89516 (specs v0.55)
