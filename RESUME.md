...
**CORRECTED MAP (successor):** BOOTLOADER 0x0-0x20000 | STATE 0x20000-0x21000 (4K, proven) | OTA-META 0x21000-0x22000 (candidate prefix+approval) | PERSONA 0x22000-0x24000 | ACTIVE 0x24000-0x91000 (436K/109pg) | DFU 0x91000-0xFF000 (440K/110pg = ACTIVE+1, the documented min) | JOURNAL 0xFF000-0x100000 (4K confirmed-floor, OUTSIDE the swap). **SUCCESSOR REQUIREMENTS (GO):** (1) ONE shared geometry source (app+boot+ota.rs+persona) + build-time overlap/boundary KATs. (2) candidate-bound approval = a DOMAIN-SEPARATED SHA-256 over the EXACT verified bytes: SHA256(DS_TAG ‖ header[137B exact] ‖ sig[64B exact] ‖ payload_digest[independently recomputed over the staged DFU payload]) — NOT the {package_id,seq,payload_hash} field-triple (coordinator correction 2026-07-13: that triple is NOT candidate-complete — an attacker keeps those 3 values + the payload but substitutes the other header fields (target_class/carrier/tg/device/payload_type/issuer_pk/policy) and/or the signature, and the approval-skip then boots an UN-verified header). Record = [APPROVED_MAGIC][approval_digest 32][phase u32][fmt/version u32][CRC32]. CHECKED write+readback, committed LAST before SWAP magic. Boot RECOMPUTES the digest from CURRENT OTA-META (header+sig) + CURRENT DFU (payload) bytes and skips the Ed25519 verify ONLY on byte-exact digest match + readback; ANY changed header/sig/payload byte ⇒ digest mismatch ⇒ full re-verify. Never enter prepare without a durable matching commit; resumed-progress requires matching approval + NEVER rejects/erases partial DFU. (3) checked reject — prove STATE non-SWAP (readback) before DFU erase, else fail-stop. (4) payload gate — 0<len≤ACTIVE_LEN, PT_FIRMWARE_FULL, bootable-vector sanity, USE VerifiedUpdate. (5) split dev-bench vs prod DeviceContext (prod = persona/board/device/battery, fail-closed if unavailable). (6) confirmed floor in the SEPARATE atomic JOURNAL page + pending/confirm lifecycle, NO floor=0 fallback. (7) H4 BPROT/ACL — NOT a permitted follow-up for a prod-compilable successor (coordinator correction 2026-07-13): the PROD bootloader build MUST apply reset-time BPROT/ACL protecting BOOTLOADER (0x0-0x20000) + PERSONA (0x22000-0x24000) so a compromised app cannot rewrite the anchor/bootloader, AND partition-wrap the app's Nvmc writes to its own regions; the ONLY unprotected build is an EXPLICITLY-NAMED dev/bench feature; a prod build without protection MUST fail-closed at COMPILE time (compile_error!/const-assert), never silently ship unprotected. Implemented WITH increment 2 (bootloader). ★ ACCEPTANCE (exact re-pass, coordinator 2026-07-13): there must be NO production path that boots with a WRITABLE bootloader/persona — every prod build either boots ACL-protected or does not compile; only the explicit dev/bench feature boots unprotected. (8) falsifiers: power-cut@every write/erase/swap-boundary, stale-approval/new-candidate, map-overlap, reject-fail, floor. LOCAL commit only → exact hive-codex re-pass. DO-NOT-FLASH + Roy tg_pk gate stand.
**SUCCESSOR PROGRESS (local-only, no push):** ✅ increment 1 = `crates/rak-geometry` (d41657f) — the shared map (C3 fix) + compile-time overlap/boundary/alignment/STATE-size KATs; compiles = map self-consistent. Local commits now: ec29da4 (re-vendor) + cad8b57 (OLD buggy gate, to be superseded) + d41657f (geometry). **NEXT increments (the safety-critical gate state-machine — do FRESH, carefully):** (2) wire rak-geometry into the bootloader (replace local consts) + redesign the gate CANDIDATE-BOUND with a DOMAIN-SEPARATED envelope digest (coordinator correction 2026-07-13 — the {package_id,seq,payload_hash} field-triple is REJECTED as not candidate-complete: an attacker keeps those 3 + the payload but swaps the other header fields (target_class/carrier/tg/device/payload_type/issuer_pk/policy/abi/battery) and/or the sig, and a triple-match skip then boots an UN-verified header). Approval record @OTA_META offset 204 (4-aligned) = [APPROVED_MAGIC u32][approval_digest 32][phase u32][fmt_version u32][CRC32 u32], where approval_digest = SHA256(DS_TAG ‖ header[137B exact from OTA-META] ‖ sig[64B exact from OTA-META] ‖ payload_digest), payload_digest = SHA256 independently RECOMPUTED over the staged DFU payload (streamed, alloc-free, same primitive as verify). Boot: SWAP pending → read current OTA-META prefix (137B header + 64B sig) + stream DFU payload → recompute approval_digest → if approval.magic==APPROVED && CRC-ok && approval_digest==recomputed → APPROVED-for-THIS-EXACT-candidate → skip Ed25519 verify (resume-safe); else FRESH/CHANGED → full verify_staged_slot → on OK compute+write approval (CHECKED write+READBACK) THEN it's committed before embassy-boot swaps; on FAIL → checked-reject. ANY single changed header/sig/payload byte ⇒ digest mismatch ⇒ re-verify (no skip). NEVER enter prepare without a matching durable approval; on resumed swap-progress with missing/mismatched approval → NEVER reject/erase (let embassy-boot resume). (3) checked reject: erase STATE → READBACK proves non-SWAP → only THEN erase DFU; else fail-stop (no DFU erase). (4) payload gate (H2): 0<payload_size≤ACTIVE_LEN (not DFU_LEN — swap copies ACTIVE only), require PT_FIRMWARE_FULL, USE the returned VerifiedUpdate (check payload_type), bootable-vector sanity (ACTIVE[0..8] plausible SP+reset). (5) dev/prod DeviceContext split (H3): prod = real persona/board/device/battery from the sealed persona + board consts, fail-closed if unavailable; dev = explicit bench wildcard. (6) confirmed floor → the JOURNAL page (0xFF000, separate erasable, NOT the OTA-META candidate page); pending/confirm lifecycle writer; NO floor=0 fallback (absent journal ⇒ fail-closed, not 0). (7) app side: build.rs memory.x FROM rak-geometry; app persona read 0x12000→PERSONA_ORIGIN; ota.rs SLOT_B_BASE→DFU_ORIGIN, capacity→ACTIVE_LEN; commit_swap(prefix→OTA_META, SWAP magic LAST). (8) H4 BPROT/ACL — MANDATORY in increment 2 (coordinator correction: NOT a follow-up). PROD build applies reset-time BPROT/ACL (protect BOOTLOADER 0x0-0x20000 + PERSONA 0x22000-0x24000, partition-wrap app Nvmc writes); DEV/bench = the explicitly-named unprotected build; a prod build lacking protection FAILS at compile time (compile_error!). nRF52840 = ACL regions (not nRF52832 BPROT); the bootloader configures ACL before load(). (9) falsifier tests: candidate-bound host KATs (stale-approval/new-candidate reject, payload>ACTIVE reject, wrong-type reject) + HEADER-FIELD-SUBSTITUTION falsifiers (same package_id/seq/payload_hash + same payload but a mutated OTHER header field — target_class/carrier/tg/device/payload_type/issuer_pk/policy/abi/battery — MUST fail the digest ⇒ re-verify, NOT skip) + SIGNATURE-SUBSTITUTION falsifier (unchanged header+payload, mutated sig byte ⇒ digest mismatch ⇒ re-verify) + payload-single-byte-flip ⇒ digest mismatch ⇒ re-verify + a power-cut simulation matrix (mock-flash crash at each write/erase/swap boundary) + the ELF alloc-free symbol gate. Then compile app+boot, LOCAL commit, exact hive-codex re-pass. cad8b57's buggy gate stays until increment 2 rewrites it.
