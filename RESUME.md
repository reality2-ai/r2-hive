...
> - **✅ FALLBACK VECTOR CONFIRMED SYNTHETIC-BY-CONSTRUCTION (unblocks supervisor's canon-capture; kills a 35-commit false-positive load).** Verified at source (dfr1195 main.rs:248-250): `TG_UUID` is a hard-coded PLACEHOLDER string ('r2tg-demo' + all-zero groups); the fallback tg = **FNV-1a-32 of that literal**; `TG_HK_DEMO = [0x5C; 32]` (repeated-byte test key). Composer independently confirmed the demo TG is **NOT minted** (no master_secret/tg_pk — the synthetic UUID maps to no real tg_pk), which is exactly why the D4-unprovision decision landed on the erase path rather than minting a demo persona. => EVERY byte of the fallback frame is synthetic: target = FNV(synthetic literal), hk = repeated-byte, origin = mac_low3-derived. Capture it as the canonical KAT; **allowlist those three constants** so sweeps stop re-flagging them.
...
> - **⚠ PHRASING HAZARD (android nearly mis-asserted off my wording — propagated fleet-wide):** the route/origin wire field is a **u16 whose VALUE is (hive_id >> 16)**. Decoders see **<D4-ORIGIN>** (or <FALLBACK-ORIGIN> fallback) **UNSHIFTED** — NOT 0xc4340000, NOT the full <D4-HIVE-ID>. My "upper-16 of the FNV-32" phrasing is accurate but invites reconstructing the shift. Say "u16 wire value = hive>>16" instead.
...
Composer delivered the shared dev-TG + RAK dev persona (PUBLIC: tg_id 730c29e7-209f-4d2e-c8fd-b68e71f5f73b, tg_pk 3992f89ba04c087e8a798cb7b693c749f080c0ce43715191619b3ba7eee00f5b; RAK mesh_pk af4a9dd5.., hive_id 39013014.., wire_id cc788b17). Persona = 336B plaintext-CBOR at /home/roycdavies/.r2-dev-trial/rak-persona.bin (SECRET soft-seal, carries DEV_SK — NOT copied/committed anywhere; referenced by path only).
...
> **RAK persona INTEGRITY VERIFIED (2026-07-14):** rak-persona.bin SHA-256 = `8d5d099f340a18c9ab1859a0e69b0492cc427f570e69e6e6f1b0b3ea8dd33834` (byte-exact match to composer's mint); tg_hash = FNV-1a-32(tg_id) = `<PROV-TG-HASH>` (matches mint). Persona flashes as-is; role record (RPF1 sensor) is a separate unread file, NOT flashed. RAK dev-trial fully flash-ready — only Roy's A8 SWD flash remains.
