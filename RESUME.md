...
- **⏳ CRITICAL #2 (NVS fail-open) — coming in the SAME platforms/esp32 pass** (core authoring carefully — security-critical Result refactor: Result/fail-closed load_anti_rollback + propagate commit/stage/clear write errors + stage {seq,payload_hash} BEFORE set_boot + reconcile after mark-valid). Both criticals land in the r2-core state BEFORE I build (espup lead time). **NO ACTION until #2 lands + the combined state is ready; then I build platforms/esp32 (C6) + resolve the partitions.csv staging gap + flash-verify on real esp32-C6.**
