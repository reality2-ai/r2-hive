...
- **NEXT (metal, Roy-gated):** build+stage the standby release ELF on alfred (canonical box; NEVER flash — Roy-only) → Roy flashes XIAO → bench idle-draw/heat (est. SX1262 warm-sleeps ~62% of each 8ms cycle; the WIN is metal-measured, NOT yet confirmed). core asks for the bench miss-rate result; a high miss-rate wants a driver-side rx_us tweak. PATH-2 (phone-coupled + deep MCU light-sleep) queued behind.
