...
- **NEXT:** specs ratifies §12.4.3.1 (asked: ratify from my candidate as-is? ungated like §12.4.3?) + core co-pins the r2-forge-emit side (native-image-136B + export names) → then BIND r2-wasm-host 1:1 (swap provisional_abi, rework spike to the real convention). core owns the r2-forge wasm codegen (analog of native vtable_for). Build-now meanwhile: B.4 TG-gated module-load gate (ABI-independent).
