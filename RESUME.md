...
- **⚠ NECESSARY CO-DESIGN (in flight):** §12.4.3 froze the NATIVE repr(C) vtable; the L2 WASM boundary needs a pinned WASM PROJECTION (wasm exports + linear-memory AbiResult layout) that HOST + r2-forge-generated-module must agree byte-for-byte (a wasm module can't return a native fn-ptr vtable). **Proposed a concrete projection to core (r2-forge authority) + specs (lead):** module exports memory + globals __r2_abi_version/__r2_abi_hash(out_ptr writes 32B)/__r2_plugin_id + funcs r2_init(result_ptr)/r2_execute(cmd,dptr,dlen,result_ptr)/r2_poll(ev_out,buf,cap)->i32/r2_name(buf,cap)->i32; inst DROPPED (one module=one plugin, Ruling 3); AbiResult = 132B host-supplied linear-mem buffer ([0]tag u8; Ok→[2..4]len u16,[4..132]data128; Err→[4..8]code u32,[8..10]desc_len,[10..74]desc64). **BIND r2-wasm-host 1:1 once core confirms/counters.** Spike (ca2b341) already proves the mechanics.
- **abi_hash v1 = c37f504d4c2a9d8c** (core landed the repr(C) types + hash at 329d708; via composer). TWO forms (do not conflate — same targeting≠authorization split as B.2.0): (1) FULL 32B SHA-256 over the canonical schema = embedded in base + every module = the fail-closed LOAD-GATE exact-match (Ruling 2); c37f504d4c2a9d8c is its first 8 bytes — **I still need the full 32B from core for the host load gate.** (2) 8B truncation c37f504d4c2a9d8c = UpdateHeader / composer-recipe COMPAT pre-filter, authenticated by the header sig (Ruling 1). Monolithic images carry all-zero (no frozen-ABI dep); only L2/L3 modules carry the real hash.
