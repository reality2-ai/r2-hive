...
## 🔥 XIAO EDGE-BRIDGE POWER-STANDBY (heat fix, Roy GO 2026-07-10, task #65) — SPEC GATE ✅CLEARED, HELD on Roy scope-eyeball
...
- **GATES: (1) spec ratify ✅ (§3.2.6); (2) Roy scope-eyeball ✅ CONFIRMED YES; (3a) core accept Diff 1/2 ✅ LANDED @1bbb32b (verified in-tree: listen_duty_cycle default lora.rs:67, 0x94+us_to_steps+Sx1262 override, 2 KATs); (3b) core accept Diff 3 (LoRaTransport mode) — ⚠ NOT LANDED, re-flagged (core busy/queued). RE-VENDOR HELD ON Diff 3.**
- **⚠ DO NOT re-vendor 1bbb32b yet:** Diff 3 (set_rx_standby/rx_duty/arm_rx on LoRaTransport) is ABSENT in 1bbb32b — lora_transport.rs still re-issues plain radio.listen(). Firmware (810573e) calls lora.set_rx_standby → won't compile without Diff 3. Core landed from the first handoff (733d82d); the Diff 3 follow-up (c7fc7a8) was queued while core was busy → re-sent core the exact Diff 3 shape, awaiting a new sha. core's 1<<5→1<<2 PreambleDetected prose fix folded (609c11e); not arming it (agreed).
