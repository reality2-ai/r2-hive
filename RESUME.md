...
**⚠ DO-NOT-MIRROR + status ambiguity (2026-07-12):** (1) **Android commit 271132f = POISONED reference — do NOT copy it.** It accidentally committed the RETIRED v0.33 host flow (wrong msg4/msg13/order/bad_key + accepts empty-DONE persistence) after v0.34 landed; Android was told to revert the whole commit. (2) **v0.34 landed-status is UNCERTAIN across the fleet:** hive-codex said "v0.34 NOT LANDED, mid-edit/uncommitted"; android-codex says "c57d568 = v0.34 landed." Do NOT start the coordinated pass off either the ambiguous status OR 271132f — android-codex confirms **coordinated implementation REMAINS HELD.** Implementation gate = v0.34 CONFIRMED clean-landed (specs, not a mistaken commit) AND the cross-impl hold explicitly lifted. Until then: verify the canonical v0.34 text directly on specs before mirroring ANY reference impl.
