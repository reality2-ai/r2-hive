...
- **NO brief, ledger entry or report may cite a log archive as if one exists** (supervisor-decided, binds now). Retrospective device questions → **measure hardware** or **UNANSWERABLE**. *"The logs show no X" is not a claim.* hive swept its own 214 tracked files: **0 violations** (one pre-policy contemporaneous console read is now permanently unverifiable — flagged, not rewritten).
