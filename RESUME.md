...
> **⚠ DO-NOT-ASSUME — CANONICAL SPEC REF (interim, supervisor 2026-07-11):** specs' full latest session output — **R2-UPDATE v0.58-0.60 (reject codes 16/17/18 @55143e0), board/chip_profile schema v0.34, TG-lane v0.59 (2f71cf4)** — is committed on specs' worktree branch **`spec-conformance-v0.2`** but **NOT pushed to origin. origin/main is STALE** (~R2-UPDATE v0.57 / R2-RUNTIME v0.30). INTERIM RULE until specs declares the durable integration branch: (1) canonical ratified spec TEXT = **`spec-conformance-v0.2` tip** (readable on Alfred); (2) do NOT verify/build against origin/main — it's behind; (3) "verify-vs-git" = that ref's tip, never handed-over-only. **Impact on my threads: PIECE-B reject codes 16/17/18 = @55143e0 on the branch (deferred anyway, so no premature build); tn_base board_profile must track schema v0.34 on the branch, not origin.** No code impact today — implementations already track the ratified text; this only fixes WHICH ref "checked-in" means. Awaiting specs' confirm that spec-conformance-v0.2 IS the integration branch (or merge+push to main) + push-to-origin for durability.
