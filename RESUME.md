...
> - **🔴 FIFTH ISSUE IS LIVE — IT BLOCKS THE FLASH (I verified for core).** The **D4 io_task EXTENDED LoRa leg is NOT dormant**: io_task IS spawned on D4 (guard = `cfg(not(feature="xiaobridge"))`, main.rs:625; D4 is fakesensor), and io_task contains **6 mesh_broadcast calls** (mesh_broadcast -> DATA_TX_LORA -> lora_route_task -> air) + **8 encode_extended calls**. So D4 puts EXTENDED frames on the SAME LoRa air as apiary's COMPACT frames, and core's WireFormat is PER-NODE => a Compact-mode RAK will **MIS-PARSE** them (extended byte0 is structurally decodable — same ver|type|flags packing — so it does NOT cleanly Err). **Must be settled BEFORE flashing.** My read for specs: R2-LORA §5.1 makes the LoRa bearer compact-only => the io_task extended LoRa leg is the NON-CONFORMANT side and the fix is MINE (gate that leg off LoRa under the demo compose, or confine io_task's extended traffic to the WiFi/UDP carrier). Routed to specs by core; I will compile-verify a gate on their ruling, not invent it.
