...
## BENCH-MESH REGRESSION INVESTIGATION (2026-07-27) — RE-DERIVED from transcript after compaction lost it

**★ NAMING (supervisor, binding): this is NOT "the #d001 relay proof." #d001 is RATIFIED PASS + CLOSED (2026-07-22: multi-hop D4-LoRa-RAK-LoRa-XIAO, route_len=2 observed, vanishing RAK-off / returning RAK-on, positive control; route_len=1 present throughout). Calling this campaign the #d001 proof re-opens a closed ratified result by loose naming = the stale-gate defect pointed at a proof. It is a bench-mesh REGRESSION INVESTIGATION until the regression-vs-retraction question (below) is answered from git, not memory.**
...
- **★ #d001 REGRESSION-vs-RETRACTION — INTERIM RULING (supervisor 2026-07-27): #d001 STANDS as a HISTORICAL PASS (not refuted), BUT its SUBJECT IS UNIDENTIFIED, so it MUST NOT be cited as evidence that the CURRENT lineage relays.** The deciding fact — the 07-22 build sha — is **UNRECOVERABLE from the ledger**: DECISIONS.md recorded the VERDICT (route_len=2, RAK-off counterfactual, positive control) but NOT the artifact; and the flash-authorization file is **NOT versioned** (`.fleet` is not a git repo; every grant overwrites the last in place), so the sha-pin existed only while the grant was live. (Supervisor owned this as the SAME class as my git-log false-denominator: no commit ⇒ no durable record.) Composer — which keeps image+package digests — was asked before declaring it unrecoverable; **digest or nothing, NO reconstruction from dates/filenames** (this campaign produced a filename-reuse collision). **PATH FORWARD = RE-PROOF, not archaeology:** unify SF → direct D4→XIAO `route_len=1` on the current branch → `route_len=2` with the RAK-off counterfactual; that settles regression-vs-retraction empirically and is faster than excavating a July build. My git evidence (below) is consistent with a lineage-divergence regression but **NOT adopted as a finding** — re-proof decides it.
  - git evidence (rak4630-fw-wt, lineage-authoritative; DATE strings unreliable — cherry-picks carry old dates): `CrossCarrier` masking MECHANISM introduced **2026-07-17** (`2f7d9866`); the `set_relay_egress(SameCarrier)` override-call exists in only TWO commits across ALL refs — `2f7d9866` (07-17, **NOT an ancestor** of the current branch) and `70f442b9` (parent=`7011934e`, current branch, landed THIS session; 07-21 date is a cherry-pick artifact). On the CURRENT lineage the override was ABSENT from `7011934e` (HEAD through 07-22) until `70f442b9`. [[positive-control-the-tree-not-just-the-tool]] (lineage beats date), [[tn-base-is-mixed-sha-assembly]].
- **D4 (DFR1195 ESP32-S3) benchsf7 image `cbd6bf67…653c`.** ARTIFACT fact: `cbd6bf67 != a23c21ea` (non-benchsf7) proves `benchsf7` is not a no-op **IN THE IMAGE** — says NOTHING about what the board executes. **BOARD fact (separate, and the one the mesh failure depends on): the flashed D4 is running SF12 (`lora_dr=0`) — `benchsf7` did NOT take on the board.** Persona = the #d001 shared bench TG (`tg_hash 0x6E31DEC6` / `wire_id 0xCC788B17` / `tg_id 730c29e7`); `baked_persona` avoids the old 0x12000 brick path.
...
— has still NEVER been reached** (needs a provisioned board — blocked on the persona-region declaration lockstep + a physical board on the bus, NOT a Roy ruling: **g27 DISSOLVED 2026-07-27, persona region RATIFIED** — 14 sectors, four in-place anchors, one moving part, byte-identical across carriers; a DFR-on-bus read + name-the-complex-hive-xiao remain PHYSICAL, not gates). Connect +
