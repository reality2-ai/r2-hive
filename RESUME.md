...
> - **★ FIFTH ISSUE — CANON ALREADY RULED IT (Roy, 2026-07-10); I am UNBLOCKED, no specs wait. I VERIFIED THE SPEC MYSELF.** R2-LORA §5.1 verbatim: *"COMPACT is canon for LoRa, including SIGNED events — extended-on-LoRa is REVERTED (Roy RULING 2026-07-10)... The extended format MUST NOT be used on the LoRa bearer... compact-only bearers (LoRa, BLE events, ESP-NOW) stay compact."* So the D4 io_task extended LoRa leg is the NON-CONFORMANT side and **the fix is MINE** — my firmware simply hasn't caught up with a 7-day-old ruling. The same section independently confirms TWO things we'd settled separately: (a) *"a relay MUST NOT transcode a signed (auth-bearing) frame compact<->extended — the HMAC tag cannot be recomputed by a relay without the origin key"* => core's native-compact-tier is CANON-MANDATED, not merely prudent; (b) compact route entries = *"bits [31:16] of the FNV-1a-32"* => **THIRD independent confirmation of upper-16** (r2-wire types.rs §3.3 + my DFR stamp + this canon).
