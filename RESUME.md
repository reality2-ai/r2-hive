...
- **NEXT on v0.8 ratify:** (1) rebind r2-wasm-host from provisional fixed `scratch` offsets → __r2_scratch_ptr()/len() reads; (2) core emits the real r2-forge .wasm (scratch region + funcs + alloc-on-std) + the AbiResult image KAT → run the INTEROP-TEST (real r2-forge module vs my host) = the true end-to-end; (3) build-now: B.4 TG-gated load gate; (4) later: browser + esp32s3+PSRAM wasm3.
