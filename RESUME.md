...
- **★ BOARD-CLASS WiFi QUESTION CLOSED (supervisor 2026-07-27, ACK on the first PINNED attestation in the chain): NOT going to Roy — no bench board joins infrastructure WiFi, nothing to except.** OPEN ITEMS remaining (both NON-hive): (1) **D4 runs something NO record explains** (SF12/`lora_dr=0`, no flash record) — the only open item on this thread; supervisor ordered COMPOSER (serial custody) to do a NON-DESTRUCTIVE boot-banner read (no flash/read_flash/grant), same instrument that identified X1. Three remedies by outcome: unrecorded-flash / wrong-record / pre-custody-carry. I correctly assert NO digest — do NOT infer one. (2) **X1 full sha died with the overwritten grant file** — composer holds an ABBREVIATED digest only; grants are now archived before overwrite (won't recur).
