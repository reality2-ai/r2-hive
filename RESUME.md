...
- **NEXT (metal, Roy-gated):** build+stage the standby release ELF on alfred (canonical box; NEVER flash — Roy-only) → Roy flashes XIAO → bench idle-draw/heat + duty-cycle-engages check (est. SX1262 warm-sleeps ~62% of each 8ms cycle; the WIN is metal-measured, NOT yet confirmed). **BENCH SIZING (core-verified, see docs/XIAO-BRIDGE-STANDBY-PLAN.md §2 "Bench sizing"):** detection rule (rxPeriod+sleepPeriod) <= TX preamble airtime; bench 3ms/5ms + 8-sym SF7 preamble (8.19ms) FIRST — rx=3ms is MARGINAL (~2.9 symbols). On high miss-rate, core's lean = (b) lengthen TX preamble to 12-16 sym BEFORE (a) bumping rxPeriod. **★ SF12 re-size before field: 8-sym SF12 preamble = 262ms = sleepPeriod cap — don't carry 3/5 to SF12** ([[setrxdutycycle-preamble-sizing]]). PATH-2 (phone-coupled + deep MCU light-sleep) queued behind.
