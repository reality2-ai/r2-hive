...
- **📦 BATCHED v0.8 (specs anti-churn): specs ratifies ONE R2-PLUGIN v0.8 = (1) globals→funcs amendment (abi_version/plugin_id, DONE + host-proven) + (2) §12.4.3.2 (scratch as funcs + reserved-region + optional alloc).** I hand BOTH inline together (specs can't read cross-repo) once core co-pins. **HELD on rebinding r2-wasm-host to §12.4.3.2 until specs ratifies v0.8 (spec-first).**
