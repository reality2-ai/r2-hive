...
- **⛔ GATE:** the ABI-bound host-import surface is HELD until specs FREEZES §12.4 → repr(C) (fn-ptr table + repr(C)/repr(u8) results + caller-provided buffers + abi_version; D.4 R2-PLUGIN row). Asked specs freeze status + offered host-implementability input (off-thread answer → inbox). A.5.3's no-borrowed-crossings + caller-buffers is ALREADY the wasm requirement → lowering is wasm-friendly by construction.
- **BUILDABLE NOW (no rework):** r2-wasm-host crate skeleton (runtime-backend trait); a wasmtime runtime-MECHANICS de-risk spike (load/instantiate/call/host-import/memory, PROVISIONAL placeholder ABI); the **B.4 TG-gated module-load gate** (verify module sig under TG update root BEFORE instantiate — a module is firmware, gets firmware sovereignty; ABI-independent, security-critical). **NEXT: the wasmtime spike + crate skeleton.**
- Reported supervisor (c709e23 plan). Coordinating under specs; separate from RAK.
