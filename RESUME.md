...
## 🔒 HOLD — P3 USB-PAIR X25519 security finding (android-codex cross-impl, 2026-07-12) — CONFIRMED real in hive; fix CANON-GATED + cross-impl-aligned
...
**➡️ v0.37 LANDED (specs origin 01a9865) = ratified 3-MESSAGE durable-journal atomic key-commit terminal (resolves the reconnect-selector gap):** PAIR_CONFIRM(7)→PAIR_DONE(8)→**PAIR_ACK(14, NEW)**, three K1-auth HMAC-SHA256[0..16] tags over the 128B transcript; bilateral durable JOURNAL {transcript,K0?,pending=K1,phase∈{CONFIRM_SENT,DONE_SENT,PROMOTED}} written BEFORE each step + crash-resumed; monotonic promotion (peripheral keeps K0 until it verifies ACK, host promotes on DONE-verify); idempotent dups; **K0 retired ONLY on a successful K1-reconnect, NEVER on TTL**; authenticated reconnect = RECONNECT_CHALLENGE carries a K1-preferring key-slot SELECTOR + host returns an authenticated acceptance (changes reconnect msg 9/10 shape). **✅ I EPHEMERALLY VERIFIED the 3 MAC constants for specs (computed, then REVERTED — NOTHING landed, tree clean): confirm=`4e4c5ff286e30e55bd71c2efdc869f4e` ✓ reproduces, done=`08ba274f802d982844df255fe5a68be8` ✓ reproduces, ack=`1ec03c3d79e1f6f19b6a797d24142d72` (NEW, label `r2-usb-pair-ack-host-v1`).** Constants confirmed: 16B truncation; labels confirm-host-v1 / confirm-peripheral-v1 / ack-host-v1; transcript = eph_pk_host‖eph_pk_peripheral‖nonce_host‖nonce_peripheral (K1-keyed). **STILL HELD (supervisor reserved the converged-GO last turn; android may still be refuting):** landing the confirm/done/ack helper + wiring the 3-msg journal state machine + reconnect selector msg 9/10 + promotion/retire + the REQUIRED state/crash KATs. Supplied specs the constants (design-input, unblocks its freeze); FLAGGED supervisor to confirm v0.37 is the convergence + give the land-GO. When GO: land helper + UP14 (incl ack) + journal + reconnect selector + KATs, one pass, cross-impl-aligned.
**✅ TENSION RESOLVED + confirm/done BYTE-LOCKED (2026-07-12): specs (v0.38 §5.3.4 + UP14, origin 5ea5575) LOCKED confirm=`4e4c5ff2…` / done=`08ba274f…` to MY verified values, and specs now EXPLICITLY AGREES to wait for supervisor's convergence signal before I land ("DO wait for convergence")** — so the specs-request-vs-supervisor-hold conflict is gone; both say hold. My ack=`1ec03c3d…` is now 3-WAY VALIDATED: my ephemeral usb_pair.rs computation == specs' provisional == android-codex's independent OpenSSL (full `1ec03c3d79e1f6f19b6a797d24142d728ec571de945ed6a094dccc002c073b0e`, trunc16 `1ec03c3d79e1f6f19b6a797d24142d72`). All 3 terminal MACs byte-agreed cross-impl. **⚠ NOT CONVERGED YET (android-codex 2026-07-12): v0.37/v0.38 are DESIGN-PARTIAL — specs still OWES (a) the authenticated reconnect selector + host-acceptance wire (msg 9/10 are still nonce-only/single-tag → lost-ACK recovery is CIRCULAR), (b) reboot-stable 60s-deadline journal semantics, (c) the state/crash KATs. Three terminal MAC bytes CANNOT freeze P3b without these.** So this is NOT the land-go (answered my supervisor go-check). My post-convergence deliverables (specs' list): (1) byte-verify confirm/done ✓done + report — reported; (2) confirm ack label+value — ack-host-v1 / 1ec03c3d… confirmed; (3) supply the reconnect selector + host-ACK msg9/10 wire; (4) state/crash KATs. HOLD stands until specs lands the reconnect schema + deadline + supervisor signals converged.
**⚠⚠ SUPERVISOR OVERRIDE (2026-07-12) — MACs are PROVISIONAL, NOT final despite the v0.38 lock; v0.37 is NOT the land-GO (my ephemeral+hold was confirmed right):** supervisor-codex's fresh v0.37 pass found a CRITICAL in the key-RETIREMENT phase logic — §5.3.4 **L558-561** (wipe pending K1 on MAC-mismatch "at ANY step") + **L580-583** (wipe on 60s terminal-miss) → with the peripheral in **DONE_SENT AFTER the host has PROMOTED**, an injected-bad-ACK OR a lost-ACK+timeout recreates a **PERMANENT K1/K0 split** — the exact desync v0.37 was meant to fix — contradicting its own never-TTL-retire rule. Phase-scoped correction routed to specs. **⇒ Treat confirm=`4e4c5ff2`/done=`08ba274f`/ack=`1ec03c3d` as PROVISIONAL** (even though specs locked confirm/done in v0.38): the correction can change the retire/promote logic + **transcript inputs**, which would change the MAC bytes. The 3-msg durable-journal COMMIT structure looks AGREED; the narrow remaining bug class = key-RETIREMENT phase logic. **LAND-GO fires ONLY after supervisor-codex + android-codex return a CLEAN pass on the CORRECTED version + supervisor pings.** Nothing landed (tree clean). "Close, not done."
**↻ v0.39 (android-codex 2026-07-12): selector SHAPE fixed, but twin pass found NEW blockers → STILL HELD, bytes STILL PROVISIONAL:** (a) stale old reconnect construction/sequence contradicts the new wire; (b) generic abort can discard post-DONE K1; (c) **`host_gens` list is NOT MAC-bound → MITM can strip K1 + force K0 recovery/DoS**; (d) gen width/list constraints + reboot deadline undefined; (e) KATs absent. Canon has now iterated v0.34→v0.39, each round surfacing a fresh blocker — the ephemeral-verify-don't-land discipline keeps being vindicated. Land only on a CLEAN twin pass (supervisor-codex + android-codex) on the corrected version + supervisor's GO.
**↻ v0.40 (android-codex 2026-07-12): new P1 host key-RETENTION contradiction → STILL HELD:** host `CONFIRM_SENT` must be durable-retained from CONFIRM send, but the spec still allows pre-DONE K1 transient-drop + pre-PROMOTED abort-wipe → if peripheral reached `DONE_SENT` and DONE is lost, Host drops K1 while Peripheral retains → **deadlock**. Fix = host retains K1 from CONFIRM send. Also UP16 (strip-attack KAT, specs/android-owned — NOT my UP14) topology invalid; correct strip-compare = P gen0/list0 `f93e…` vs H gen0/original-list01 `deb8…` (not gen1/list01 vs gen0/list0). Canon now v0.34→v0.40; all bytes + KATs PROVISIONAL. My UP14 (confirm/done/ack) unchanged this round but stays provisional (retention/transcript still in flux).
**✅ TERMINAL MACs RE-LOCKED (specs v0.41 §5.3.4 + pair-vectors 0.9, origin c24cfa9, 2026-07-12) — MY VALUES STABLE, no longer provisional:** confirm=`4e4c5ff2…`/done=`08ba274f…`/ack=`1ec03c3d…` re-locked in canon because I confirmed (impl-owner) the **terminal domain binds NO gen/phase/host_gens** (transcript = eph_pk_host‖eph_pk_peripheral‖nonce_host‖nonce_peripheral, 128B, K1 — mirrors sas_code). The 7-round v0.34→40 churn was ALL in the SEPARATE reconnect domain (msg 9/10/15), never the terminal MAC — my ephemeral-verified constants were the stable anchor that let specs re-lock. **⏳ IMPLEMENTATION still HELD pending a ROY A/B SCOPING CALL (supervisor paused the crash/rotation-durability grind):** **Option A = ship happy-path first-pair + SIMPLE same-key reconnect NOW** (single link_key, the original gen-free reconnect_tag, NO generations/rotation/journal), defer durability to a field-hardening track → **this is ~all the pairing crypto I ALREADY HAVE (usb_pair.rs + usb.rs) + the 3 terminal MAC helpers, MINUS the rotation state machine** (small, low-risk); **Option B = hold for full convergence** (dual-key/journal/rotation). specs pings the MOMENT Roy rules → I implement to the PINNED profile, not a guess. usb_link_id/UP13 (R2-KEYSTORE info-independent device identity) already aligned.
