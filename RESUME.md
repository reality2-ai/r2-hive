...
- **⛔ SPEC-FIRST BLOCKER (path-1 flash gated):** dc-advertise §12.6 + SCF-1 §3B.2 for a duty-cycled NEXT-HOP bridge (D4 SCF-HOLDS OUTBOUND to the bridge's advertised wake window). Net-new bit: §3B.2 today = sleeping ORIGINATOR; here = sleeping TRANSIT next-hop. **Handed specs the contract Q (off-thread answer→inbox); if net-new I draft the §ref for specs ratify.**
- **TWO DEPENDENCIES before path-1 impl:** (1) Roy scope-eyeball (reported supervisor); (2) specs ratify the SCF contract; (3) core add SetRxDutyCycle. **HELD until Roy approves scope + specs ratifies — do NOT start deep impl or trigger core driver work prematurely.**
