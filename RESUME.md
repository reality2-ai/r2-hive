...
- **★ FLASHABLE .bin — RULED (supervisor 2026-07-27): option (a), APP IMAGE not merged; THREE parties — Roy PRODUCES (runs the one save-image via `!`, ungated), hive ATTESTS, composer WRITES.** Granter (supervisor) must NOT produce what it pins; hive is gated; ⇒ Roy produces = cleaner separation than the original plan (arrived at because the gate over-matched). App-only (ota_0 @0x20000), no merge (merge would write bootloader+table from 0x0 = forbidden). Artifact to produce: `~/x1-otav3-A-baked-app.bin`. **AWAITING Roy's run**, then hive attests (sha256+size, byte-search ON THE .bin, extract/rehash/offset, DROM prediction treated as prediction; STOP+report if padded/absent/offset-wrong). No-hand-roll refusal accepted on the record (a home-built header/checksum ≠ byte-identical = false pin). Prior HELD detail retained below.
