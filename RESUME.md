...
**★ composer confirmed (2026-07-13): trailer re-staging keeps its wire stream UNCHANGED (receiver owns staging; ping only if header-sig framing changes — it won't). The fresh bench TG is ROY-GATED by construction** — R2-KEYSTORE §4 requires an interactive no-echo custody passphrase (deliberately NOT scriptable), so composer can't mint the TG_SK without Roy at the console (composer task 37). Composer will send TWO things when Roy enters it: the 32-byte tg_pk + the tg_id (= DERIVED trust_group_uuid(tg_pk), NOT 0xF305FE07 — the placeholder is retired since the persona was never anchored). I then anchor that tg_pk @0x22000 at commissioning; issuer_pk=it; verify-header accepts (no role-0x05 cert). So the tg_pk is a Roy-console dependency (not a composer-work dependency). NET RAK-OTA gates: (1) core's layout ruling [core], (2) re-vendor r2-update@7093c9d+r2-trust@3e15d75 [me, independent], (3) tg_pk [Roy-console via composer], (4) mint (all-wildcard, ready). Gate wiring = re-vendor + core layout, then compile; metal #49 also needs the tg_pk anchored.
...
> **RAK persona INTEGRITY VERIFIED (2026-07-14):** rak-persona.bin SHA-256 = `8d5d099f340a18c9ab1859a0e69b0492cc427f570e69e6e6f1b0b3ea8dd33834` (byte-exact match to composer's mint); tg_hash = FNV-1a-32(tg_id) = `0x6e31dec6` (matches mint). Persona flashes as-is; role record (RPF1 sensor) is a separate unread file, NOT flashed. RAK dev-trial fully flash-ready — only Roy's A8 SWD flash remains.
...
OPEN (light, honest refuter): confirm the RAK is a dev-TG MEMBER (baked persona PARSED -> keyed identity) vs a fail-closed keyless repeater — the partial beacon capture shows the repeater CLASS but not clearly the persona tg_hash 0x6e31dec6; verify via keyed dev-TG traffic delivery or a member-marked beacon field (headless board = no console).
