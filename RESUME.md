...
- **✅ specs: §12.4.3.1 candidate RECEIVED IN FULL, RATIFIES NEXT TURN (QUEUED not dropped)** — folding it with the §12.4.3 reconciliation (naming ABI_HASH_FULL 32B load-gate + 8B truncation pre-filter) + R2-COMPILE §11 wasm-host seam + Level-0 tier. **SPEC-FIRST HOLD (specs instruction): do NOT bind r2-wasm-host to the final layout until specs posts the ratified §12.4.3.1; ca2b341 spike + provisional_abi stay as-is; nothing frozen ahead of ratification. specs PINGS me to bind.** (I am already holding — aligned.)
- **NEXT (fully converged, HELD on specs' ratify-ping):** on specs' §12.4.3.1 ratification ping → BIND r2-wasm-host 1:1 (swap provisional_abi → the §12.4.3.1 abi module, rework spike to the real 8-export/136B-native-image convention) + interop-test a real r2-forge-emitted module against my host when core's wasm-emit is ready. **Build-now meanwhile (ABI-independent, not held): B.4 TG-gated module-load gate.**
