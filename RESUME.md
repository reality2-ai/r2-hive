...
**ARCH (in flight):** peripheral SM = no_std xiaobridge fw. usb_pair.rs crypto = RustCrypto (hkdf/hmac/sha2/x25519-dalek, all no_std-able) → fw reimpl on its vendored crypto, byte-verified against the shared vectors (conformance = reproduce UP1-8/13/14/18 exactly). Explore agent mapping fw crypto/CBOR/NVS/USB-control-frame surface. **Vectors:** UP1-8,13,14,18 ACTIVE; UP9-12,15-17 NON-ACTIVE/historical (durability track).
