...
## 🔨 BUILDING — P3 USB-PAIR Profile A (Bench) — supervisor GO 2026-07-12 (canon @specs 6b4cb80 R2-PROVISION v0.42); HOLD LIFTED
**★ GO (supervisor): build the PERIPHERAL side of pairing Profile A (Bench).** Canon verified on specs origin/main HEAD **6b4cb80** (v0.42 = 6f8e2ad): Profile-A carve (first-pair CONFIRM→DONE→ACK byte-identical to B + simple same-key gen-free reconnect UP18, NO generations/rotation/journal), **LOCAL REQUIRED POLICY** anti-downgrade (bench requires A, no wire-negotiation, fail-closed), UP18=`2f62edaa`, + the 3 hive-codex fixes — all present. Profile B (durable journal/rotation/reconnect msg9/10/15/crash KATs) = DEFERRED field track, DO NOT build now. **r2-hive usb.rs = the HOST** (line 1 "Host-side R2-USB v2 framer"; sends CONFIRM/ACK, receives DONE) → the PERIPHERAL SM is a COUNTERPART, built in r2-hive-bin (canonical, std-testable; composer vendors to the XIAO hw build per task #62). Sent supervisor a non-blocking interpretation heads-up.
**✅ STAGE 1 DONE + committed 9114254 (vector-verified vs landed origin, local — CI down):** in `usb_pair.rs` — `confirm_tag`/`done_tag`/`ack_tag` (HMAC-SHA256(K1, label‖128B-transcript)[..16]; labels confirm-host-v1/confirm-peripheral-v1/ack-host-v1) + `shared_secret`→`Option` (None = non-contributory Z = UP14 `bad_key` reject; host usb.rs:812 call site now aborts bad_key) + `reconnect_tag` unchanged = UP18. New test `profile_a_terminal_reconnect_and_reject_vectors` asserts byte-exact: k1=`386667c2…`, confirm `4e4c`/done `08ba`/ack `1ec0`, reconnect `2f62edaa`, zero-key→None. **Full hive-bin lib 111/111 green.** ⚠ do NOT hardcode-and-claim — regenerate `gen_usb_pair_vectors.rs` + vendored fixture + coverage floor in this pass (hive-codex guardrail 2).
**⏳ STAGE 2 IN PROGRESS = the PERIPHERAL first-pair SM** (receive HELLO_HOST(4)→send COMMIT(5)→receive HOST_REVEAL(13)→send REVEAL(6)→receive CONFIRM(7)+verify→send DONE(8)→receive ACK(14)+verify→promote) + SAS operator-confirm (peripheral) + simple same-key reconnect (msg9={nonce_rc}/msg10={tag}) + local-required-policy A (fail-closed) + the guardrails (name lost-DONE/ACK/reboot recovery; SAS timeout pre-CONFIRM only, no post-CONFIRM destructive wipe; no opportunistic A/B fallback). Then regen vectors + coverage. NEXT: read specs §5.3.4 Profile-A peripheral flow + host usb.rs SessionState/frame-builders → build the PeripheralSession.
