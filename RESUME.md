...
**PATTERN-SET DIFF (hive gate vs composer's 3 gates, 2026-07-27):** ★my one-off g23 enumeration ≠ my GATE — `public-hygiene.sh` scans NO IPs (no RFC1918, no CGNAT), no UUIDs, no API-keys (composer's device-id/key gates do). Composer's documented KNOWN-LIMIT (partial-MAC/mac_low3 tails) is what MY gate covers, along with terms/macron/gateway-product/host-names. **OWED (on supervisor go-ahead, needs KATs): add IP(RFC1918+CGNAT)/generic-UUID/API-key classes to public-hygiene.sh** (false-positive mgmt is the hard part: semver≠IP, commit-SHA≠UUID, git-check discriminator). Coverage-parity applies unconditionally to DEVICE/SECRET classes (IP/UUID/key/MAC/tail); the naming classes (pilot/cultural/gateway) are per-corpus, may stay hive-only. Rule: a gate narrower than its cross-repo sibling is a hole that shows up only when the value lands in the narrower tree — unify COVERAGE, not tools.
