...
1. **No contributory-key rejection.** `usb_pair::shared_secret` (usb_pair.rs:68-73) returns `*z.as_bytes()` with NO `z.was_contributory()` check. A zero/low-order `peer_pk` forces Z=0 → SAS + link_key become attacker-known (MITM defeat of the "MITM-protected" wired pairing). **LIVE-REACHABLE:** consumed at usb.rs:812 (`let z = usb_pair::shared_secret(&p.eph_sk_host, &pk_periph)`) inside the `UsbSession` P3 state machine — NOT dormant primitives. UP1-13 vectors lack a negative case.
...
**DECISION = HOLD, do NOT patch unilaterally (spec-first + cross-impl parity).** The rejection semantics need canon — specs/android escalation ACTIVE to pin was_contributory-rejection in R2-PROVISION §5.3.4 + a negative KAT. Patching hive's `shared_secret` API (→Result/Option) now would DIVERGE from Android's approach + pre-empt the canon. **When canon lands: (a) reject `!was_contributory()` at the primitive, align the abort path with Android, (b) add the negative KAT vector (low-order peer_pk → reject), (c) `Zeroizing<>` the stored Z/link_key/eph_sk + enable the x25519-dalek zeroize feature — ALL in one coordinated pass, both impls together.** Confirmed both gaps to android-codex; holding P3 state-machine hardening until canon pins rejection. Folds around the P2 v2-conformance work (same usb.rs surface).
