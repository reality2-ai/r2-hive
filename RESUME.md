...
- **⚠ FILE-SIZE TRAP FOR THE NEXT WRITER:** this file sits **under 1 KB below the 65536 B hard limit — over 98 % full.** The next substantive addition will breach it. **Compact into `RESUME-archive.md` BEFORE adding**, not after; measure with `wc -c RESUME.md` rather than trusting this line. *(Stated without an exact byte count on purpose: the first version of this warning quoted one, and the commit that added the warning changed the file's size, so the figure was wrong the moment it was written. A self-invalidating number is worse than a range.)*
