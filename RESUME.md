...
- **★ FLASHABLE .bin BLOCKED (supervisor 2026-07-27: grant must pin the .bin the flasher writes, not the ELF):** hive CANNOT produce the flashable image. `espflash save-image` (pure file-write, no port/board) is intercepted by the fleet firmware/key gate (keyword match on `espflash`); NO ungated fallback (esptool absent — hive's ONLY ESP image builder is espflash). Will NOT hand-roll the ESP image (a non-espflash header/checksum/SHA-append ≠ byte-identical to the flasher's output = a false pin) and will NOT bypass the gate ([[espflash-trips-firmware-gate]], [[espflash-gate-bypassed-by-file-and-remote-exec]]). **Escalated to supervisor** — options: (a) a human (Roy) approves ONE `espflash save-image` for hive (file-only) so hive attests the real bytes + byte-searches the .bin; (b) supervisor directs otherwise. Partition table (`partitions.csv`): app `ota_0` @ **0x20000**, r2cfg 0x11000 protective. Prediction (to CHECK not inherit): persona is in a PT_LOAD DROM `.rodata` segment ⇒ save-image copies it verbatim ⇒ 336 B blob WILL appear in the .bin; empirical check waits for the real bytes. Grant pins the .bin digest, deferred.
