...
- **★ REGRESSION-vs-RETRACTION of #d001 — git evidence (rak4630-fw-wt, lineage-authoritative; commit DATE strings are unreliable in this fleet, cherry-picks carry old dates):** the `CrossCarrier` egress MECHANISM (what masks cross-PHY relay) was introduced **2026-07-17** (`2f7d9866`), BEFORE the 07-22 proof. The `set_relay_egress(SameCarrier)` OVERRIDE-call in rak4630 `main.rs` exists in only TWO commits across ALL refs: `2f7d9866` (07-17, on a lineage that is **NOT an ancestor** of the current branch) and `70f442b9` (parent=`7011934e`, the current branch — its 07-21 date is a cherry-pick artifact; it landed as HEAD replacing `7011934e` only THIS session). **On the CURRENT branch lineage the override was ABSENT from `7011934e` (the HEAD through 07-22) until `70f442b9`.** ⇒ **the deciding fact I CANNOT derive from rak4630-fw = which image/branch the 07-22 #d001 proof was BUILT from** (that sha is in the ledger, supervisor's): if built from the current-branch 07-22 HEAD (no override → `CrossCarrier` default → masked) the ratified proof is IN QUESTION; if built from the `2f7d9866` lineage (override present) #d001 STANDS and the current-branch masking is a lineage/re-vendor REGRESSION. **Lean (not a ruling — a ratified result is not mine to re-open):** consistent with a lineage-divergence regression ([[shared-checkout-path-dep-coupling]] VENDORED-vs-LIVE, [[tn-base-is-mixed-sha-assembly]]). Handed to supervisor; the 07-22 build-sha settles it.
