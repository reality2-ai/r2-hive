...
## 🔨 PREP — P3 USB-PAIR: Roy pivoted to SIMPLE-SECURE pairing (full durability DROPPED for bench, USB transitional→on-board); crypto RESTORED, SM held for specs' simple-secure §5.3.4
**★ GO #3 (Roy EXPLICIT, supervisor 2026-07-12) — SIMPLE secure pairing; full durability DROPPED for the bench** (the USB link is transitional → on-board soon, so the Profile-B durability grind isn't worth it for the bench). **✅ PREP DONE — RESTORED the parked terminal-MAC crypto to the tree (cherry-pick 9114254 → commit 5fc3a20):** confirm_tag/done_tag/ack_tag + `shared_secret`→Option non-contributory reject + host bad_key abort; **re-locked/4-way-validated, verify GREEN vs current landed vectors (UP14 terminal + reconnect 2f62edaa), full lib 111/111.** **⏳ HELD for specs' clean ping — do NOT build until specs lands simple-secure §5.3.4** (current specs canon is STILL v0.44 = Option B @5c551e1; simple-secure NOT written yet). When it lands: build the simple PERIPHERAL SM (HELLO→COMMIT→HOST_REVEAL→REVEAL→CONFIRM→DONE→ACK first-pair + persistent link_key + simple same-key reconnect + **re-pair-on-doubt** in place of durable recovery; NO generations/journal/rotation/crash-recovery) + regenerate vectors from the simple spec (coverage floor, not hardcoded) + reconcile the `profile_a`-era naming. **⚠ hive-codex revector caveat:** if the simple-secure §5.3.4 rewrite bumps KDF domains/version, the terminal MACs change → re-verify before the SM build. The full-durable Profile-B blocker list below is now BENCH-DROPPED (kept as reference IF field durability returns as a later track). GO history: A GO→retracted → B (Roy explicit) → **SIMPLE-secure (Roy explicit, current)**.

## (was 🔒 HELD Profile B) — P3 USB-PAIR: Roy chose PROFILE B (full durability); Profile-A GO RETRACTED 2026-07-12
**⚠ WHIPSAW RESOLVED — Roy EXPLICITLY chose Option B via a direct AskUserQuestion (AUTHORITATIVE, supervisor 2026-07-12); this SUPERSEDES ALL prior Profile-A authorizations** (supervisor's inferred-A GO, supervisor-codex's re-issued A GO, AND the specs "Roy-delegated-via-carry-on" auth — the carry-on was inference, now explicitly OVERRULED). Security-first: complete durable pairing before ANY ship; NO Profile-A implementation until further notice. The v0.42 Profile-A carve stays LANDED as spec text but is NOT to be implemented. Profile B = the ACTIVE BLOCKING critical path (specs drives to convergence). **supervisor confirmed "keep the stable crypto/usb_pair assets, they're reusable" — my STAGE-1 terminal-MAC crypto is preserved in git @9114254 (parked, NOT in the working tree; the pre-existing usb_pair helpers are untouched); I will NOT restore it to the tree until the Profile-B GO.** Sequence for the record: supervisor GO'd Profile A (auth inferred from Roy's "carry on") → I verified canon (specs v0.42 @6b4cb80) + built **STAGE 1** (terminal MAC helpers confirm/done/ack + `shared_secret`→Option non-contributory reject) vector-verified byte-exact (committed 9114254) → supervisor CORRECTION: **Roy picked B, not A** → I **REVERTED STAGE 1** (revert commit 3fff533; usb_pair.rs/usb.rs/generator byte-identical to pre-GO b16b992; lib back 110/110 green; tree clean). **DO NOT build the peripheral pairing SM.** HOLD until FULL durable pairing (Profile B) converges + is twin-clean; supervisor pings the real GO then. supervisor-codex's parallel Profile-A GO was superseded by supervisor's Roy-explicit correction.
**Re-use note:** the STAGE-1 terminal MAC helpers are byte-identical across Profile A+B (gen-independent terminal transcript) + the non-contributory reject is a both-profile MUST — the tested code is preserved at **9114254** in git history to cherry-pick when Profile B is GO'd (all 4 MAC values still 4-way validated: confirm `4e4c`/done `08ba`/ack `1ec0`, reconnect `2f62edaa`). Do NOT re-land ahead of the Profile-B GO.
**hive-codex v0.42 blockers (why A wasn't buildable anyway — feed into Profile B):** (a) Profile A at 465-470 says no journal but INHERITS the unqualified durable-journal MUST (566-614) + generations (616+); no explicit lost-DONE/ACK/reboot persistence/manual recovery; (b) local-only/no-wire profile can't detect an A/B mismatch during the byte-identical first-pair → field doesn't fail closed until reconnect after keys persist; (c) **global unknown-key skip means A accepts a B msg9 key1 nonce + skips host_gens key2 = directly reinterpreting the B shape** → needs a fixed required-profile assertion/discriminator (NOT negotiation/fallback) or distinct types + profile-qualified sequencing; (d) vector defects: conformance omits UP18, UP14 durability applicability unclear, metadata calls gen-free UP9-12 "generated" while current UP9-12 are gen-bound. All routed to specs.
**★ AUTHORITATIVE CURRENT CANON = R2-PROVISION v0.43 @ `1b6eaefcb4e45d42cb7636e3b1a249fcee9fe06f`** (supervisor-codex 2026-07-12) — **RETRACTS Profile A**, requires Option B full durability as the active android+XIAO blocking path; supersedes v0.42 (6f8e2ad = normative v0.42 + vectors; 6b4cb80 = the RESUME pointer — identity drift RESOLVED). v0.43 resolves the Profile-A review defects (the pre-review found **8 normative defects** in the A carve despite crypto passing — critical = byte-identical first-pair with no negotiation can't make a B peer reject A pre-activation; the unconditional journal/gen/grace/crash MUSTs still applied to A; msg9/10/15 table conflict; reconnect only authenticates peripheral→host; ACK-loss+peripheral-crash no recovery) **by REMOVING A, not patching it.** **HIVE: do NOT build A.** Held until full generation-reconnect + bilateral durable journal + rotation + crash/loss/reboot semantics converge + pass review. **✅ my tree independently verified CLEAN by hive-codex** (@3514e10, diff b16b992..HEAD shows NO pairing delta — 9114254 fully reverted by 3fff533; Option-B hold recorded). **⚠ PROFILE-B open CRITICAL (hive-codex, feed the convergence): REVEAL→peripheral-crash durable deadlock** — P sends PAIR_REVEAL then crashes before journaling CONFIRM/DONE_SENT; H journals CONFIRM_SENT K1 + sends CONFIRM; P reboot drops transient K1/transcript, can't verify the replay; H post-boundary may never wipe K1 → **permanent split**. FIX = P retention boundary must be BEFORE REVEAL (REVEAL can cause H to commit) + durable pre-terminal phases/replay + H resumable pre-terminal state + an AUTHENTICATED cancel for SAS-decline once P is durably prepared. **↳ SHARPENED (supervisor-codex 2026-07-12, product-state counterexample — the claimed bilateral invariant is NOT satisfied):** if BOTH sides lose power after host CONFIRM_SENT/CONFIRM but before the peripheral journals DONE_SENT, host reboots holding K1 while peripheral has NEITHER K0 NOR K1/transcript → resent CONFIRM can't verify, reconnect intersection empty, host can't wipe K1 = **permanent split.** Structural fix (converges with the above): **the peripheral MUST durably journal {transaction, pending K1} BEFORE the host may cross CONFIRM_SENT** — e.g. before PAIR_REVEAL as readiness evidence, or an authenticated persisted-ready step. Requires a **simultaneous-power-loss KAT at EVERY interval.** Also v0.43 stale lines 740-744 still call A buildable + vector JSON unchanged (UP18 active/non-provisional) — specs owes. Spec source pin going forward = **v0.43 1b6eaef** (not v0.42/6f8e2ad).
**⛔ v0.43 RESIDENT REVIEW VERDICT (supervisor-codex 2026-07-12): Option B NOT IMPLEMENTABLE — specs must revise; Hive HELD. The Profile-B convergence blocker set:** **CRITICAL** — (1) max-gen reset→gen1 can't activate/retire the old max under higher-gen-only rules → need authenticated **lineage_id/epoch** in metadata+MACs + atomic reset + old-lineage discard KATs; (2) lower-gen recovery re-rotation lacks a bound **target_gen** → peers derive the same key but label it different gens → host-selected target_gen must be journaled + MAC/frame-bound; (3) stale-A residue: active conformance still requires non-provisional UP18 + status still says A buildable → delete active-A residue / exclude historical UP18. **HIGH** — (4) no complete bilateral reconnect phase / write-before-send / activate / retire table; host never verifies ACCEPT despite both-retire wording; duplicate ACCEPT conflicts with generic out-of-state error; (5) UP9-17 reconnect/state/crash vectors still provisional + no exact msg9/10/15 CBOR frames; (6) **hive_id vs usb_link_id conflict remains** (my usb.rs concern — earlier "UP13 aligned" but the formula-input ambiguity persists); (7) **R2-USB v2→v1 fallback can BYPASS the pairing activation gate → strict durable pairing MUST refuse v1 activation** (HIVE-specific: my host usb.rs negotiates down to v1 — `negotiates_down_to_v1_when_peripheral_responds_v1`; Profile B must fail-closed on v1). **MED** — canonical shortest CBOR / unique keys / exact field-type-length / no-trailing-data; fix TV25 reservation text. Product-state proof ABSENT. All feed the Profile-B build when specs revises + a clean review lands.
