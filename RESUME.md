...
- **NEXT:** specs lands §12.4.3.1 (design already endorsed + inline text delivered — just needs write access) + core co-pins the r2-forge-emit side (native-image-136B + export names; core also starting r2-forge __r2_plugin_vtable_v1 codegen as the M1 follow-on) → then BIND r2-wasm-host 1:1 (swap provisional_abi, rework spike to the real convention). Build-now meanwhile: B.4 TG-gated module-load gate (ABI-independent).
