...
## 🤝 COMPOSER coordination (answered, awaiting core): RAK tn_base pin (specs §3.3.1 tn_base@ver)
Composer finalizing RAK recipe/v2; asked if `r2-rak4630@0.1.0` is the tn_base pin. ANSWERED: confirmed the model (platform-binary 0.1.0 = build-OUTPUT realizing the shared TN stack, NOT the tn_base pin; repeater role intrinsic, function_ensembles empty), BUT flagged the build-reality caveat — no unified `core-TN-base` crate in the RAK build; it's ~9 individually-VENDORED core crates (path-deps) at MIXED shas (r2-trust/r2-update @a83b167, r2-ble @d80ebf7, others older). So `core-TN-base@1.0.0` can't be rubber-stamped — core/specs must MAP the RAK's vendored SHAs to the ratified base version + coupling-KAT (piecemeal re-vendor → SHAs likely LAG 1.0.0). Offered composer/core the full SHA manifest. [[tn-base-is-mixed-sha-assembly]]. **Awaiting: core's base-version mapping for the RAK's SHAs.**
