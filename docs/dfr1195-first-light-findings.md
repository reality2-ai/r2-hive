...
Board connected (on **<rig-host>** `/dev/ttyACM0`); hive on **<build-host>** (esp/Xtensa toolchain), passwordless
SSH → build-on-<build-host> / flash-on-<rig-host>. This is the metal-validation half of the core↔hive D3b split:
...
- <build-host>: `esp` toolchain (espup) + Xtensa GCC (`~/.rustup/toolchains/esp/xtensa-esp-elf/.../xtensa-esp32s3-elf-gcc`); `export-esp.sh` at `~/Development/homelab/export-esp.sh` — **must be sourced before building** (sets PATH/LIBCLANG).
- <rig-host>: cargo 1.95 + espflash 4.4.0 + cargo-espflash, board at `/dev/ttyACM0`, `roycdavies` in `dialout`. (workshop's sensor rig + :21042 dashboard also on <rig-host> — flashing `/dev/ttyACM0` only, no service restarts.)
...
- **Alternative:** pin an older espflash (pre-descriptor-enforcement) just for this build — but workshop's rig uses 4.4.0 on <rig-host>; don't disturb. Prefer the version bump.
